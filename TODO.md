...
- [x] **P0** iOS buildに含まれるQRC画像1,289件とinstall-only画像32件を、`ios-image-licenses.json`の明示台帳と埋め込みlicense metadataで分類し、未分類0件を監査する。QRCの内訳はCC-BY-SA-4.0機能画像796件、Breeze/Oxygen LGPL画像319件、Android画像2件、KXmlGui画像1件、project-wide GPL fallback画像148件、LibrePaint CC0ブランド画像23件。install-only画像は19個のCMake定義から毎回再収集する。ビルド`20260807151823`では実機上のLibrePaintロゴ、About画面、Dockアイコンを確認済み。2026-08-09に最新構成のbaseline build、arm64最終リンク、runtime stagingをhost上で再確認し、実機での全画面確認は継続する。
