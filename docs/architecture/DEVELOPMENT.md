...
公開ヘッダー、クラス、プラグインと、16の中核所有ターゲットの直接依存および利用元を
各台帳から再生成する。
...
接続面単位の許可依存を保持する。次のコマンドは、16の中核所有ターゲット間の直接リンクを
...
全互換経路のR1-G7削除条件、全304件の逆方向includeと627件の内部ヘッダー参照の
...
- `kritaimage`と`kritaui`について、公開マクロを持つ製品ヘッダーと所有元外から直接
  includeされる製品ヘッダーの和集合が、欠落と余分な項目なしで記録されている。
- 所有ターゲットが記録した全プラットフォームに存在し、宣言と実装が所有元の
  ソースディレクトリーに属する。
- 公開根拠が公開マクロの宣言、または所有元外の全利用ソースによる直接includeと一致する。
...
- 16の中核所有ターゲットが5構成のいずれかに存在し、定義場所、種別、製品ターゲットへの
...
- 16ターゲット間の35リンクが、共有ターゲットの全責務を含む85候補へ欠落なく射影され、
  同一責務内9候補、許可方向52候補、R1-G4で基準化する24候補へ分類される。
- 許可方向外の責務対のうち現在の5責務対が257件の直接includeへ一意に帰属し、元の
  CMakeターゲット辺と5構成へ接続される。
- 確認済み5責務対が理由、所有段階、除去条件、現在件数と等しい審査済み上限を持つ。
- 残る8責務対が直接includeの実責務へ帰属し、逆方向依存ではない射影として解決される。
- 16中核ターゲットと全製品構築ターゲットが、5構成すべてで強連結成分0件を維持する。
