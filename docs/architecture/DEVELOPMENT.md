...
公開ヘッダー、クラス、プラグインと、15の中核所有ターゲットの直接依存および利用元を
各台帳から再生成する。
...
接続面単位の許可依存を保持する。次のコマンドは、15の中核所有ターゲット間の直接リンクを
...
- 15ターゲット間の27リンクが、共有ターゲットの全責務を含む69候補へ欠落なく射影され、
  同一責務内8候補、許可方向35候補、R1-G4で基準化する26候補へ分類される。
...
変更した場合は方針を編集してから許可依存方針の更新器を実行する。`verify-quick`は6更新器の
`--check`、台帳検査器、欠落、所有者、公開根拠、利用根拠、責務分類、ターゲット関係、
