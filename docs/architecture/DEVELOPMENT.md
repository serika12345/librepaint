...
全互換経路のR1-G7削除条件、全311件の逆方向includeと32件の内部ヘッダー参照の
...
- 18ターゲット間の39リンクが、共有ターゲットの全責務を含む94候補へ欠落なく射影され、
  同一責務内14候補、許可方向58候補、R1-G4で基準化する22候補へ分類される。
...
- 18中核ターゲットと全製品構築ターゲットが、5構成すべてで強連結成分0件を維持する。
- 公開宣言を持たない14ヘッダー、32件のパッケージ外参照が、所有段階、理由、除去条件、
  ヘッダー数と参照数の審査済み上限を持つ。
