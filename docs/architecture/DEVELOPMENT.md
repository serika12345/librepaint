...
- 運用検査スクリプト自身の単体試験
- 10責務の再配置先、移行履歴、互換経路、依存方向、循環、公開ヘッダー境界
- シェルスクリプト
...
`docs/architecture/package-relocation-plan.json`は、10責務の目標ディレクトリー、名前空間、
CMakeターゲット、8段階の移行順、`kritaui` CMake互換経路、各依存基準の縮小上限を保持する。
次のコマンドは、責務地図と許可依存方針、5構成のCMakeターゲット台帳へ計画を照合する。
...
検査器は、未実装または新規ターゲットを含む責務対について許可依存の下位側を先に移す順序、
現行、新規、実装済みターゲットの区別、`kritaui`互換経路のR1-G7削除条件を確認する。
両責務のターゲットが実装済みの場合、移行段階は実施履歴として扱い、現行の責務層と
有向非巡回グラフを依存順の正本とする。責務、公開接続面、ターゲット台帳を変更した場合は、
先行する台帳を更新してから再配置計画を同じ変更で更新する。

既存の公開大域C++識別子は確立済みのAPI・ABI名として維持する。新しい公開APIは
再配置計画の責務名前空間へ置き、所有ターゲットの公開面へ登録する。
...
- 14所有ターゲットの未宣言内部ヘッダーとパッケージ外参照が各0件である。
- 10責務の目標パッケージと8移行段階を記録し、未実装責務の順序、実装済み責務の現行DAG、
  互換経路、逆方向依存、内部ヘッダー参照のゼロ上限を検査する。
...
パッケージ依存と構造依存の直接検査を実行する。責務の目標配置、移行順、作成ターゲット、
互換経路、または段階別上限が変わる場合は再配置計画も更新する。
`verify-quick`は6更新器の`--check`、公開面、責務地図、許可依存、直接依存、構造依存、
再配置計画の検査器と、欠落、所有者、公開根拠、利用根拠、責務分類、ターゲット関係、
