...
公開ヘッダー、クラス、プラグインと、22の中核所有ターゲットの直接依存および利用元を
各台帳から再生成する。
...
接続面単位の許可依存を保持する。次のコマンドは、22の中核所有ターゲット間の直接リンクを
...
- 22の中核所有ターゲットが5構成のいずれかに存在し、定義場所、種別、製品ターゲットへの
...
- 22ターゲット間の65リンクが、共有ターゲットの全責務を含む129候補へ欠落なく射影され、
  同一責務内21候補、許可方向78候補、R1-G4で基準化する30候補へ分類される。
