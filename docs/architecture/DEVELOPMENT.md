...
公開ヘッダー、クラス、プラグインと、19の中核所有ターゲットの直接依存および利用元を
各台帳から再生成する。
...
全互換経路のR1-G7削除条件、全311件の逆方向includeと現在28件の内部ヘッダー参照の
...
- `kritaimage`、`kritaimpex`、`kritaimpexui`、`kritaui`について、公開マクロまたは
...
- 19の中核所有ターゲットが5構成のいずれかに存在し、定義場所、種別、製品ターゲットへの
...
- 19ターゲット間の52リンクが、共有ターゲットの全責務を含む100候補へ欠落なく射影され、
  同一責務内14候補、許可方向63候補、R1-G4で基準化する23候補へ分類される。
...
- 19中核ターゲットと全製品構築ターゲットが、5構成すべてで強連結成分0件を維持する。
- 公開宣言を持たない`kritaui`の11ヘッダー、28件のパッケージ外参照が、所有段階、理由、除去条件、
  ヘッダー数と参照数の審査済み上限を持つ。
