...
- macOSの`develop`を`bc545362`までGit履歴として`ssh nixos`上の
...
- Linux実機の永続Ninja木で`KisStrokeJobContractTest`、`kis_vec_test`、
  ...
  `KoInsetsContractTest`、`KisSpacingInformationContractTest`だけを限定構築した。各CTestの単発実行と20回
  反復が成功し、直近の契約と構築分離がmacOSとLinuxで一致した。変更なし構築閉包はLinuxでケージ座標
  試験が5工程・16入力、
  ...
  1工程・3入力、図形余白試験は5工程・13入力、描画間隔試験は4工程・13入力である。全ネイティブ検証は
  実行していない。
