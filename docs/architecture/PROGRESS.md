...
- 更新日時: 2026-09-21 18:16 JST
- 状態: `in_progress`
- 現在の検査段階: R2-G19at 間隔設定画面依存の直接化
- 関連TODO: R2-G19a・R2-G19b・R2-G19c・R2-G19f・R2-G19g・R2-G19h・R2-G19i・R2-G19j・R2-G19k・R2-G19l・R2-G19m・R2-G19n・R2-G19o・R2-G19p・R2-G19q・R2-G19r・R2-G19s・R2-G19t・R2-G19u・R2-G19v・R2-G19w・R2-G19x・R2-G19y・R2-G19z・R2-G19aa・R2-G19ab・R2-G19ac・R2-G19ad・R2-G19ae・R2-G19af・R2-G19ag・R2-G19ah・R2-G19ai・R2-G19aj・R2-G19ak・R2-G19al・R2-G19am・R2-G19an・R2-G19ao・R2-G19ap・R2-G19aq・R2-G19ar・R2-G19as・R2-G19at完了、R2-G19au・R2-G19e・R2-G19d-a・R2-G19d-bは`planned`
- ブランチ: `issue-44-direct-dependencies`
- 開始コミット: `9ec942f8b8`。作業開始時点の作業ツリーは変更なし。
- 目的: 間隔設定画面が、集約ライブラリーから偶然得るQt画面部品、曲線設定、間隔モデル、入力状態、MOC処理とリンク実装に依存する状態を解消する。
- 範囲固定: `plugins/paintops/libpaintop/KisSpacingOptionWidget.{h,cpp}`、`KisSpacingOptionModel.{h,cpp}`、同ディレクトリーのCMake対象、および間隔入力を保存する専用CTestに限定した。間隔入力、設定画面、プリセット設定、描画結果は維持する。
- 調査: 初期の`misc-include-cleaner`は画面の公開ヘッダーと実装で14件、画面専用モデルで1件の直接取込み不足を報告した。モデルは7種類を混在させる`kritapaintopoptionmodelobjects`に含まれ、画面を単独で構築しても無関係なモデルを再構築していた。間隔画面の操作結果を保護するCTestは存在しなかった。
- 完了: 公開ヘッダーと実装を標準関数、Qt画面部品・メタオブジェクト、設定型、翻訳、曲線設定、間隔データとモデル、画面接続、Lagerの所有ヘッダーへ直接接続した。間隔画面とその専用モデルを`kritapaintopspacingoptionwidgetobjects`へ移し、Qt Core・Widgets、曲線設定画面、間隔データ、Lagerを公開利用要件、翻訳と画面接続を実装専用依存へ分離した。MOCは専用対象で実行し、集約ライブラリーは同じオブジェクトを一度だけ取り込む。`KisSpacingOptionWidgetContractTest`は実画面で有効化した等方間隔とダブ間更新が、プリセット設定の`Spacing/Isotropic`と`PaintOpSettings/updateSpacingBetweenDabs`へ保存されることを検証する。
- 検証: `run-shared-test-env`経由で専用対象と`kritalibpaintop`を成功させ、macOSパッケージ境界は1,739対象を確認した。公開ヘッダー、実装、モデル、契約CTestの`misc-include-cleaner`は警告0件である。Ninja閉包は専用対象1,994工程、最終ライブラリー2,132工程である。`KisSpacingOptionWidgetContractTest`と`KisBrushPresetDynamicsCompatibilityTest`は各1件成功した。`verify-quick`は45件の方針試験、責務・公開契約・テスト契約・統治・文書検査を含めて成功した。
- 残るリスク: 実行検証はmacOS・Qt 6.11.1に限る。Qt 5、Linux、Windows、Androidの実行確認はR2-G19dで扱う。残る設定画面実装は未監査である。
- 次の作業: R2-G19atを一変更化し、シャープネス設定画面を次の有限な監査単位として確定する。
