...
多角線操作状態の移設をコミットした後、`libs/ui/tool/KisToolOutlineBase.{h,cpp}`を起点に、自由形状の
点列、継続入力、取消し状態を`libs/tools`へ移す。入力座標変換、輪郭表示、キャンバス更新、操作完了の
接続はUI所有へ残し、通常入力、継続入力、点の取り消し、完了、取消し契約を観測可能な試験で固定する。
