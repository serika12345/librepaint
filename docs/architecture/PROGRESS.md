...
- 更新日時: 2026-08-16 19:45 JST
- 状態: `planned`
- 現在の検査段階: R1-G5 再配置計画
- 関連TODO: `docs/architecture/TODO.md`の「R1: コードパッケージングの改善」
- ブランチ: `r1-g4b-structural-violation-baseline`
- 目的: R1-G5aとして現在の責務、目標ディレクトリー、名前空間、CMakeターゲット、
  移行順、互換経路、削除条件を機械可読な再配置計画へ固定する。
...
- 初回契約検査は構造依存基準検査器の欠落を診断し、高速検査の統合契約は構造依存基準
  更新器の`--check`が未接続であることを診断した。
- `nix develop .#test --command ./scripts/verify-quick`: 77件の単体試験、公開ヘッダー、
  UI直下クラス、UIツールクラス、プラグイン、責務地図、許可依存方針、依存違反基準の
  決定的更新検査、構造依存基準の射影、循環、内部ヘッダー検査、公開面検査、運用検査、
...
- `nix develop .#test --command ./scripts/architecture/verify_cmake_graphs.py
  ...
  /home/masato/worktrees/librepaint-r1-g4b-verify`: macOS、Linux、iOS、Android、Windowsの
...
R1-G5aとして再配置計画の正本と最初の移行波を定義する。

1. 9責務について、現在の所有ディレクトリーとターゲットから、目標ディレクトリー、
   名前空間、CMakeターゲットへの対応表を作る。
2. 許可依存の層、8種類305件の逆方向依存、44ヘッダー627件の内部参照から、依存下位側を
   先に安定させる移行順と各段階の基準縮小条件を決める。
3. 公開識別子、転送ヘッダー、ターゲット別名、プラグイン登録への互換影響を列挙し、
   各一時経路に所有者と削除条件を設定する。
4. 特性試験が存在し、独立したCMake構築と基準縮小を同じ変更で確認できる最小の移行波を
   R1-G5bまたはR1-G6の先頭候補として選ぶ。
5. 決定的更新器と検査器を高速検査へ接続し、5構成の実体との一致を確認する。
