...
- 更新日時: 2026-08-16 16:29 JST
- 状態: `planned`
- 現在の検査段階: R1-G3 責務地図と依存方針
- 関連TODO: `docs/architecture/TODO.md`の「R1: コードパッケージングの改善」
- ブランチ: `r1-g2e-plugin-inventory`
- 目的: R1-G3aとしてアプリケーション、文書、入出力、入力、ツール、キャンバス、描画、
  リソース、プラグイン基盤の現在の所有者と責務を機械可読な地図へ固定する。
...
- 初回契約検査はプラグイン全件採取入口の欠落を診断し、高速検査の統合契約は
  プラグイン更新器の`--check`が未接続であることを診断した。
- `nix develop .#test --command ./scripts/verify-quick`: 49件の単体試験、公開ヘッダー、
  UI直下クラス、UIツールクラス、プラグインの決定的更新検査、公開面検査、運用検査、
...
- `nix develop .#test --command ./scripts/architecture/verify_cmake_graphs.py
  ...
  /home/masato/worktrees/librepaint-r1-g2e-verify`: macOS、Linux、iOS、Android、Windowsの
...
R1-G3aとして現在のパッケージ責務地図を作成する。

1. この文書の状態を`in_progress`へ変更し、R1-G1のCMake依存グラフとR1-G2の公開面、
   主要クラス、UIツールクラス、プラグイン台帳を入力にする。
2. アプリケーション、文書、入出力、入力、ツール、キャンバス、描画、リソース、
   プラグイン基盤の責務ID、現在の所有ディレクトリー、CMakeターゲットを定義する。
3. 各責務を公開面、主要クラス、プラグインサービス、直接依存、利用元へ接続した
   機械可読な責務地図を追加する。
4. 未知の責務、所有者の欠落、CMakeターゲットと台帳参照の不一致を診断する契約試験を追加する。
5. 責務地図と検査器を実装して全検証を成功させ、R1-G3bの許可依存方向を次の作業にする。
