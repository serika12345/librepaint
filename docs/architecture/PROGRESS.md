...
次の未対応ヘッダー`libs/ui/nodes/kis_node_model.h`のうち、全体選択表示の取得・設定、サムネイル寸法、待機作業管理器、
ダミー木有無を担う5 APIを次の小単位とする。既存`kis_node_model_test`の変更なし計画、直接CMake依存、空構築閉包を
監査し、専用契約が製品`kritaapplicationui`へ直結しない構成を先に作る。表示方式アダプター欠落時の既定値と設定
抑止、表示方式への値委譲、サムネイルキャッシュへの寸法・待機作業管理器委譲、ダミー木有無を具体的なモデル私有
状態から分けて挙動契約を追加する。
