...
`libs/global/KisPortingUtils.h`の4 APIについて、表示環境を必要とする画面番号取得と、文字列・UTF-8処理を
一つの独立試験へ収めた場合の構築閉包を先に確認する。SSH鍵エージェントの復旧後、未同期契約を
`ssh nixos`上のLinuxへ同期する。
