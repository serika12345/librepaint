...
- macOSの`develop`を`3be3d1f1`までGit履歴として`ssh nixos`上の
...
- Linux実機の永続Ninja木で`KisStrokeJobContractTest`、`kis_vec_test`、
  ...
  `kritapigmentcmykmathsobjects`、`KoCmykColorSpaceMathsContractTest`だけを限定構築した。
  ...
  10入力、コンテナー補助試験、整数演算試験、不透明度端点試験は各4工程・10入力、CMYK尺度定数の製品
  実装対象は1工程・3入力、同試験は5工程・21入力である。全ネイティブ検証は実行していない。
