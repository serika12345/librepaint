...
`libs/ui/nodes/kis_node_model.h`の公開クラス、コンストラクター、デストラクターを次の小単位とする。現在の木構造契約と
既存モデル試験の変更なし計画、直接依存、空構築閉包を比較し、複製列数の正規化、更新圧縮器とサムネイルキャッシュの接続、
索引変換器と非公開状態の破棄順を固定する。具象所有の初期化と破棄に必要な閉包が大きい場合は、寿命規約の観測点を先に分ける。
