...
- 第50便の専用作業ツリーはまだ作成しておらず、読み取り専用監査中の追加構築物はない。計画コミット後に担当用3本だけを作成し、各cleanコミットの
  統合直後に対応する作業ツリーを削除する。
...
第50並列便の計画をコミットし、担当用3作業ツリーを作成する。4新規限定対象が未知対象として失敗することを記録してから並列実装を開始する。
各担当は対象の単発・20回反復CTest、軽量近傍、限定閉包、製品共有ライブラリー非接続、公開API契約検査、`verify-quick`を確認する。
