...
- macOSの`develop`を`bcc2af08`までGit履歴として`ssh nixos`上の
...
- Linux実機の永続Ninja木で`KisStrokeJobContractTest`、`kis_vec_test`、
  ...
  `KoLabColorSpaceMathsContractTest`だけを限定構築した。
  ...
  尺度定数の製品実装対象は各1工程・3入力、各試験は5工程・21入力である。全ネイティブ検証は実行して
  いない。
