...
- macOSの`develop`を`1513a322`までGit履歴として`ssh nixos`上の
...
- Linux実機の永続Ninja木で`KisStrokeJobContractTest`、`kis_vec_test`、
  ...
  `KoCmykColorSpaceTraitsContractTest`、`kritapigmentlabmathsobjects`、
  ...
  1工程・3入力、CMYK画素特性試験は6工程・23入力、灰色・BGR画素特性試験は各4工程・18入力である。
  全ネイティブ検証は実行していない。
