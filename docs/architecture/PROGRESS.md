...
`libs/ui/nodes/kis_node_model.h`の項目データ役割列挙と18列挙値を次の小単位とする。現在の表示状態契約を最寄りの
専用対象として、新規ヘッダー契約の直接CMake依存と空構築閉包を先に測る。Qt利用者役割から始まる連続値、将来拡張用の
予約値、サムネイル寸法を符号化する開始値を公開モデル規約として固定し、製品`kritaapplicationui`へ接続しない。
