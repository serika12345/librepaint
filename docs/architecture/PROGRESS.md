...
- 更新日時: 2026-08-16 18:57 JST
- 状態: `planned`
- 現在の検査段階: R1-G4 既存違反基準と継続検査
- 関連TODO: `docs/architecture/TODO.md`の「R1: コードパッケージングの改善」
- ブランチ: `r1-g4a-dependency-violation-baseline`
- 目的: R1-G4bとして共有ターゲットに残る6未確定射影を解消し、ターゲット循環と
  内部ヘッダー参照の基準範囲を機械可読に定義する。
...
- 初回契約検査は依存違反基準検査器の欠落を診断し、高速検査の統合契約は依存違反基準
  更新器の`--check`が未接続であることを診断した。
- `nix develop .#test --command ./scripts/verify-quick`: 71件の単体試験、公開ヘッダー、
  UI直下クラス、UIツールクラス、プラグイン、責務地図、許可依存方針、依存違反基準の
  決定的更新検査、公開面検査、運用検査、シェル検査、文書検査、リンク検査、
  D2再生成検査が成功した。
...
- `nix develop .#test --command ./scripts/architecture/verify_cmake_graphs.py
  ...
  /home/masato/worktrees/librepaint-r1-g4a-verify`: macOS、Linux、iOS、Android、Windowsの
...
R1-G4bとして未確定射影と残る構造違反の基準範囲を定義する。

1. `libs/ui/widgets/KoStrokeConfigWidget.h`の責務を分類し、キャンバス表示から共有`kritaui`へ
   向かう4未確定射影を実際の公開面へ帰属させる。
2. `kritaui`から`kritaimpex`へのリンクをソース所属とシンボル利用まで調べ、入力および
   ツールから入出力への2候補を確認済み違反または保守的射影として解消する。
3. 15中核ターゲットと対象を拡張した製品ターゲットについて強連結成分を採取し、現在の
   循環を所有者、最大範囲、除去条件へ接続する。
4. 公開面台帳の対象外ヘッダーを別パッケージから参照する箇所の採取規則を定義し、
   確認済み内部ヘッダー参照を審査済み基準へ記録する。
5. 新規・拡大を継続検査で診断し、R1-G4の残りまたはR1-G5の再配置計画を再開地点にする。

## R1-G4a完了根拠

- 確認済み8責務対が全305件の直接includeと元のCMake辺へ対応付く。
- 各違反が理由、所有段階、審査済み上限、除去条件を持つ。
- 共有ターゲットによる6未確定射影が確認済み基準から分離され、根拠不足の状態を保持する。
- 違反増加、根拠変更、上限縮小を高速検査で診断できる。
- 5構成の実体、許可依存方針、依存違反基準の一致を同時検証できる。
