...
- 更新日時: 2026-09-19 23:32 JST
- 状態: `in_progress`
- 現在の検査段階: R2-G19c 利用者から観測できる振る舞いを守るテストへの整理
- 関連TODO: R2-G19a・R2-G19b完了、R2-G19cテスト整理、R2-G19d対象OS検証
- ブランチ: `develop`
- 開始コミット: `4ec204cf13`。共有ライブラリー境界の復旧と設定・画面接続の振る舞い試験を確定済み。
- 目的: 実行時の観測結果を持たない型特性・完全signature専用試験を、機械的な基準で試験集合から除去する。
- 完了: `ContractTest.cpp`のうち、型特性または`static_assert`を含み、Qt Testの実行時検証を一つも持たない251件と専用CMake対象を削除した。
- 次の作業: 実行時検証と型特性検査が混在する189件から、型特性だけの試験関数を除去し、観測結果を検証する部分を残す。
- 検証: macOSのCMake再構成が成功し、CTest登録は1,020件になった。削除済みソースへのCMake参照は0件で、高速検査も成功した。

## 現在の変更範囲

機械削除の条件は、ファイル名が`ContractTest.cpp`で終わり、`type_traits`または型特性の
`static_assert`を含み、`QVERIFY`、`QCOMPARE`、`QTRY_*`、`QFAIL`、`QSKIP`、
`QEXPECT_FAIL`、`QBENCHMARK`による実行時検証を一つも持たないことである。
この条件に一致した251ファイルは、通常の利用側をコンパイルすれば検出できる宣言形状だけを固定していたため、
独立した代替試験を追加せず削除した。各専用対象のinclude、compile definition、UI生成を含むCMake定義も除去した。
...
`ctest --preset tdd-macos -N`は1,020件を登録した。

## 残る課題と再開条件

型特性と実行時検証が混在する189件はファイル単位で削除しない。
型特性だけの試験関数、宣言、補助マクロ、不要になった`type_traits`依存を除去し、
値、状態変化、通知、外部への副作用を検証する試験関数を残す。
通常の数値・テンプレート処理に型特性を利用する23件はこの機械削除の対象外とする。
