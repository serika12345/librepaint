...
- macOSの`develop`を`42f1710c`までGit履歴として`ssh nixos`上の
...
- Linux実機の永続Ninja木で`KisStrokeJobContractTest`、`kis_vec_test`、
  ...
  `KoColorProfileConstantsContractTest`、`KisImageSignalsContractTest`だけを限定構築した。各CTestの
  単発実行と20回反復が成功し、直近の契約と構築分離がmacOSとLinuxで一致した。変更なし構築閉包は
  Linuxでケージ座標試験が5工程・16入力、
  ...
  1工程・3入力、画像信号試験は5工程・13入力である。全ネイティブ検証は実行していない。
