...
- 更新日時: 2026-09-21 21:36 JST
- 状態: `in_progress`
- 現在の検査段階: R2-G19bk ライブラリー製品ソースの一括直接取込み
- 関連TODO: R2-G19aからR2-G19bkまで完了、残るプラグイン群とR2-G19dは`planned`
- ブランチ: `issue-44-direct-dependencies`
- 開始コミット: `e36fe7f3bd0`。作業開始時点の作業ツリーは変更なし。
- 目的: `libs/`の製品翻訳単位を一括監査し、別の公開・実装ヘッダーから偶然得ていた標準・Qt・製品型の所有ヘッダーを直接取り込む。
- 範囲固定: 試験・ベンチマークを除く1592翻訳単位を対象とし、固定CTest、公開API、保存形式、描画結果を維持する。
- 調査: 8並列の`misc-include-cleaner`を1592翻訳単位へ一括適用し、1493実装を機械修正した。全体構築の失敗だけを反復抽出し、完全型、継承変換、集約ヘッダーが必要な誤削除を局所的に復元した。
- 完了: 標準ライブラリー、Qt値型・メタオブジェクト、画像・ノード・資源・画面・MLT型の所有ヘッダーへ1493実装を直接接続した。静的検査が扱えない共有ポインターの完全型とBoost・MLTの集約取込みはコンパイラー診断を優先した。
- 検証: 既存の共有Nix環境とNinja木だけを使った`ninja -C build/tdd-macos -k 100 all`が成功した。固定テスト配下の開始基準からの差分はゼロである。
- 残るリスク: 実行検証はmacOS・Qt 6.11.1に限る。残るプラグイン、Qt 5と各OS構成は未完了である。
- 次の作業: `plugins/paintops`と`plugins/tools`を除く残りのプラグイン製品翻訳単位へ同じ一括処理を適用する。
