...
- 次の作業: `KisLocalStrokeResourcesContractTest.cpp`について、実行中ストロークの資源スナップショットの追加・削除・複製・型別解決の利用結果と、内部コンテナー内容、回復可能アサート回数、共有ポインターの所有形状を固定する検証を整理する。
