...
- 実装結果: 350ミリ秒以内、開始距離の65%以下、回転10度以内、解放前の戻り8%以内をクイックピンチとする。二指拡大縮小・回転は更新中の既存処理を維持し、二指の解放時に現在角度を45度境界で最寄りの90度単位へ整列した全体表示を求める。開始時の角度・倍率・画像中心位置から目標状態までを180ミリ秒の`OutCubic`曲線で同時に補間し、倍率には幾何補間を用いる。完了時は既存の`ZOOM_PAGE`を適用して最終状態を厳密に合わせ、新しい指操作ではアニメーションを現在位置で中断する。三指への変更、取消し、遅い操作では判定を破棄する。Qt 6では各接触点の押下時刻を使用し、未提供時とQt 5では単調時計へ戻る。
