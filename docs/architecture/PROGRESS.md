...
同じ`libs/ui/nodes/kis_node_manager.h`に残る19 APIのうち、活動レイヤーまたは表示画像から参照画像を作る
3 APIを次の小単位とする。`kis_node_manager.cpp`の参照画像作成入口について、対象指定の変更なし計画、直接
CMake依存、空構築閉包を監査し、入力投影の選択、画像変換、既存参照画像の後へ置く重なり順、追加命令とツール
切替え、作成失敗時の入力別通知を具体的な文書・キャンバス・画面効果から分けて挙動契約を追加する。
