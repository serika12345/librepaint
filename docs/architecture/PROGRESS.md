...
同じ`libs/ui/nodes/kis_node_manager.h`に残る28 APIのうち、選択ノードの切取り・複写と貼付けを担う3 APIを
次の小単位とする。`kis_node_manager.cpp`のクリップボード入口について、対象指定の変更なし計画、直接CMake依存、
空構築閉包を監査し、親なしノードの除外、切取りと複写の区別、貼付け位置と適用器の引渡しを具体的な画像・
クリップボード処理から分けて挙動契約を追加する。
