...
同じ`libs/ui/nodes/kis_node_manager.h`に残る6 APIのうち、クラス、生成・破棄、表示接続、画面更新を担う5 APIを
次の小単位とし、操作登録を担う`setup()`は独立した後続単位とする。`kis_node_manager.cpp`のライフサイクル処理に
ついて、対象指定の変更なし計画、直接CMake依存、空構築閉包を監査し、再選択接続の所有、旧表示からの切断、
新表示への接続、保存済み活動ノードと画像末尾ノードへのフォールバック、資源提供者への活動通知、レイヤー・
マスク管理の画面更新委譲、私有状態の1回解放を具体的な表示・画像・管理器から分けて挙動契約を追加する。
