...
- 更新日時: 2026-09-21 18:09 JST
- 状態: `in_progress`
- 現在の検査段階: R2-G19as 複数入力モデル依存の直接化
- 関連TODO: R2-G19a・R2-G19b・R2-G19c・R2-G19f・R2-G19g・R2-G19h・R2-G19i・R2-G19j・R2-G19k・R2-G19l・R2-G19m・R2-G19n・R2-G19o・R2-G19p・R2-G19q・R2-G19r・R2-G19s・R2-G19t・R2-G19u・R2-G19v・R2-G19w・R2-G19x・R2-G19y・R2-G19z・R2-G19aa・R2-G19ab・R2-G19ac・R2-G19ad・R2-G19ae・R2-G19af・R2-G19ag・R2-G19ah・R2-G19ai・R2-G19aj・R2-G19ak・R2-G19al・R2-G19am・R2-G19an・R2-G19ao・R2-G19ap・R2-G19aq・R2-G19ar・R2-G19as完了、R2-G19at・R2-G19e・R2-G19d-a・R2-G19d-bは`planned`
- ブランチ: `issue-44-direct-dependencies`
- 開始コミット: `5e7d47a894`。作業開始時点の作業ツリーは変更なし。
- 目的: 複数入力選択画面が使う入力モデルが、集約ライブラリーから偶然得るQtモデル、入力ID、Lager状態、MOC処理とリンク実装に依存する状態を解消する。
- 範囲固定: `plugins/paintops/libpaintop/KisMultiSensorsModel.{h,cpp}`、同ディレクトリーのCMake対象、および利用者が観測する入力一覧の専用CTestに限定した。公開API、入力選択、共有・個別曲線、設定画面、プリセット設定、描画結果は変更しない。
- 調査: 初期の`misc-include-cleaner`は公開ヘッダーで6件、実装で27件の直接取込み不足を報告した。実装は集約ライブラリーだけに収容され、選択画面が単独の直接リンク要件を持たなかった。既存のモデル契約CTestは存在しなかった。
- 完了: 公開ヘッダーと実装を標準pair・vector・探索、Qtモデル・索引・値・文字列・メタオブジェクト・signal、入力ID、Lagerカーソルの所有ヘッダーへ直接接続した。実装を`kritapaintopmultisensorsmodelobjects`へ分離し、公開利用要件をQt Core、入力ID、Lagerへ限定した。MOCは専用対象で実行し、複数入力選択画面はその実装対象を直接リンクし、集約ライブラリーは同じオブジェクトを一度だけ取り込む。`KisMultiSensorsModelContractTest`は入力名・有効状態・ID解決の表示結果と、最後に有効な入力を無効化できない遷移を検証する。
- 検証: `run-shared-test-env`経由で専用対象と`kritalibpaintop`を成功させ、macOSパッケージ境界は1,737対象を確認した。公開ヘッダー、実装、契約CTestの`misc-include-cleaner`は警告0件である。Ninja閉包は専用対象4工程、最終ライブラリー2,130工程である。`KisMultiSensorsModelContractTest`と`KisCurveOptionModelTest`は各1件成功した。`verify-quick`は45件の方針試験、責務・公開契約・テスト契約・統治・文書検査を含めて成功した。
- 残るリスク: 実行検証はmacOS・Qt 6.11.1に限る。Qt 5、Linux、Windows、Androidの実行確認はR2-G19dで扱う。残る設定画面実装は未監査である。
- 次の作業: R2-G19asを一変更化し、間隔設定画面を次の有限な監査単位として確定する。
