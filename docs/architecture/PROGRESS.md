...
- 公開API契約は503件、未対応基準は28,510件になった。macOSで2対象の構築、実行、各20回反復、
  直接公開API契約検査が成功した。製品実装、公開API、実行時分岐は変更していない。Linux、
  `verify-quick`、全ネイティブ検証はまだ実行していない。
...
所有期間を専用契約で固定する。先に現在の入出力フィルター2対象を`ssh nixos`の実機で構築・各20回反復し、
Linux閉包と直接公開API契約検査を記録する。
