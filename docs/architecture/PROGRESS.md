...
同じ`libs/ui/nodes/kis_node_manager.h`に残る10 APIのうち、UI由来と非UI由来のノード起動入口、および起動通知2件を
担う4 APIを次の小単位とする。`kis_node_manager.cpp`の起動入口と共通処理について、対象指定の変更なし計画、直接
CMake依存、空構築閉包を監査し、画像グラフから外れたノードの無効化、同一ノード拒否、図形・画素ツール切替え、
非表示ノード拒否、最後の活動ノード、UI向け通知と一般通知の順序、画面更新を具体的なダミー木・ツール管理・
キャンバス表示から分けて挙動契約を追加する。
