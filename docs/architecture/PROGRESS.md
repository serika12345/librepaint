...
- 共有試験環境の`PATH`契約修正は完了した。次は第31並列便の候補を依存閉包の小さい順に確定し、担当票を記録してから専用作業木を作る。
...
第31並列便の候補について製品と最近傍試験の依存閉包を計測し、各担当の公開API集合、許可パス、直接依存、限定対象の停止上限、隣接試験、
統合順を確定する。担当票を記録した共通基準コミットから専用作業木を作成し、並列実装を開始する。
