...
同じ`libs/ui/nodes/kis_node_manager.h`に残る22 APIのうち、通常グループとクリッピンググループの作成、選択
ノードのグループ解除を担う3 APIを次の小単位とする。`kis_node_manager.cpp`のクイックグループ入口について、
対象指定の変更なし計画、直接CMake依存、空構築閉包を監査し、活動ノードと移動可否の前提、生成名、クリッピング
用マスクレイヤー、グループ解除失敗時の通知を具体的な画像・一括操作から分けて挙動契約を追加する。
