...
- 初回契約検査は公開ヘッダー採取入口の欠落と台帳スキーマ1を診断し、高速検査の
  統合契約は更新器の`--check`が未接続であることを診断した。
- `nix develop .#test --command ./scripts/verify-quick`: 37件の単体試験、決定的更新検査、
  公開面検査、運用検査、シェル検査、文書検査、リンク検査、D2再生成検査が成功した。
...
- `nix develop .#test --command ./scripts/architecture/verify_cmake_graphs.py
  ...
  /home/masato/worktrees/librepaint-r1-g2b-verify`: macOS、Linux、iOS、Android、Windowsの
...
R1-G2cとして`libs/ui`直下の公開クラスと実際の責務を台帳化する。

1. この文書の状態を`in_progress`へ変更し、R1-G2bの公開ヘッダー集合を候補入力にする。
2. `libs/ui`直下の公開ヘッダーから、公開マクロを持つクラス宣言と対応する実装を全件採取する。
3. 各クラスをアプリケーション調整、文書管理、入出力調整、ウィンドウ、表示、資源調整へ
   分類し、`KisApplication`、`KisDocument`、`KisImportExportManager`を受理例にする。
