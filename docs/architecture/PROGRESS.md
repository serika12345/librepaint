...
- macOSの`develop`を`bea6a334`までGit履歴として`ssh nixos`上の
...
- Linux実機の永続Ninja木で`KisStrokeJobContractTest`、`kis_vec_test`、
  ...
  `KoCanvasResourceIdsContractTest`だけを限定構築した。各CTestの単発実行と20回反復が成功し、直近の
  ...
  10入力、キャンバス資源ID試験が4工程・10入力である。全ネイティブ検証は実行していない。
