...
- 更新日時: 2026-08-19 09:07 JST
- 状態: `planned`
- 現在の検査段階: R1-G6e文書取り消し境界の計画
- 関連TODO: `docs/architecture/TODO.md`の「R1: コードパッケージングの改善」
- ブランチ: `r1-g6e-document-undo-plan`
- 目的: `libs/ui/kis_document_undo_store.*`を起点として、文書全体への依存を取り消し履歴の
  借用へ狭め、`libs/document/undo`の`kritadocument`を成立させる最初の独立単位を、
  実装開始前に計画する。
...
- 現行の文書状態分類は21クラスではなく25クラスである。文書寿命に加えてノード操作、
  選択操作、表示モデルが含まれるため、25クラスを一括して文書ターゲットへ移すことを
  完了条件にせず、実際の所有責務を確認して文書状態、文書表示、別機能へ割り当てる。
- 最初の起点を`libs/ui/kis_document_undo_store.*`とする。この接続は`KisDocument`全体を
  保持するが、利用する機能は取り消し履歴だけである。構築時に履歴を直接借用することで、
  取り消し接続から文書UIへの依存を除去する。
- 実装では`libs/document/undo`に`kritadocument`を作成し、履歴操作と変更通知を所有させる。
  `KisDocument`は履歴を先に構築して接続へ渡し、履歴が接続より長く存続する責務を持つ。
  リポジトリー内の利用元は同じ変更で正規ヘッダーへ更新し、旧ファイル、転送ヘッダー、
  旧クラス名の別名を残さない。
- 契約試験は、空履歴の現在操作、命令追加、直前命令の取消し、マクロの一括化、
  やり直し履歴の破棄、履歴位置変更の同期通知、同一スレッドと借用寿命を検査する。
- macOSとx86_64 Linuxで契約試験と全ネイティブ試験を実行する。macOS、Linux、iOS、
  Android、Windowsでは`kritadocument`、`kritaui`、製品利用元の構築とCMake台帳を
  同一コミットから検証し、全製品ターゲットの循環0件を維持する。
- この単位は取り消し履歴の所有境界だけを扱う。文書識別、変更状態、保存、自動保存、
  回復、文書情報、ノードと選択の操作は後続レビュー単位とする。利用事例の登録構造と、
  ドメイン計算からI/Oを分ける専用移行は開始しない。
- 完了条件は、取り消し接続が`KisDocument`と`kritaui`を参照せず、`kritadocument`が
  LibrePaint内では`kritapaintingundo`だけを下位依存として構築でき、既存の文書・画像の
  取り消し挙動を維持することである。履歴操作、通知、寿命のいずれかが固定できない場合は
  配置変更前に停止する。
