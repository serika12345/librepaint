...
- macOSの`develop`を`a0de4906`までGit履歴として`ssh nixos`上の
...
- Linux実機の永続Ninja木で`KisStrokeJobContractTest`、`kis_vec_test`、
  ...
  `KritaContainerUtilsContractTest`、`TestKoIntegerMaths`だけを限定構築した。
  ...
  10入力、コンテナー補助試験と整数演算試験は各4工程・10入力である。全ネイティブ検証は実行していない。
