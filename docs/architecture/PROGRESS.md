...
- 更新日時: 2026-09-21 18:42 JST
- 状態: `in_progress`
- 現在の検査段階: R2-G19av 固定CTestの製品利用要件直接化
- 関連TODO: R2-G19a・R2-G19b・R2-G19c・R2-G19f・R2-G19g・R2-G19h・R2-G19i・R2-G19j・R2-G19k・R2-G19l・R2-G19m・R2-G19n・R2-G19o・R2-G19p・R2-G19q・R2-G19r・R2-G19s・R2-G19t・R2-G19u・R2-G19v・R2-G19w・R2-G19x・R2-G19y・R2-G19z・R2-G19aa・R2-G19ab・R2-G19ac・R2-G19ad・R2-G19ae・R2-G19af・R2-G19ag・R2-G19ah・R2-G19ai・R2-G19aj・R2-G19ak・R2-G19al・R2-G19am・R2-G19an・R2-G19ao・R2-G19ap・R2-G19aq・R2-G19ar・R2-G19as・R2-G19at・R2-G19av完了、R2-G19au・R2-G19e・R2-G19d-a・R2-G19d-bは`planned`
- ブランチ: `issue-44-direct-dependencies`
- 開始コミット: `d7c4c5a814`。作業開始時点の作業ツリーは変更なし。
- 目的: 固定された既存CTestが、テスト側の探索パスとリンク指定ではなく、それぞれの製品オブジェクト対象が提供する直接利用要件だけで構築できるようにする。
- 範囲固定: `plugins/paintops/libpaintop/CMakeLists.txt`の airbrush、color、color source、composite operation、painting mode、precision、filter、texture I/O、texture LOD の9オブジェクト対象に限定した。`tests/`配下のソース、CTest登録、fixture、baseline、公開API、描画結果は変更しない。
- 調査: テスト側の依存追加を除いた最初の`KisAirbrushOptionDataContractTest`は、`kis_paintop_settings.h`を発見できずに失敗した。各固定CTestは対象実装と同じ設定・画像・色・外部型を直接読むが、その探索パスと利用要件が製品対象の`PRIVATE`側に留まっていた。
- 完了: 9対象の公開利用要件を、固定CTestが実際に直接使う全体基盤、画像・ブラシ、色、Qt、KDE翻訳、Boost、Eigen、Imath、OpenEXRの所有ディレクトリーと対象へ明示した。テストCMakeへの依存追加とこの作業中に追加したCTestを除去し、テスト側の開始コミットとの差分をゼロに戻した。
- 検証: `KisBrushPresetDynamicsCompatibilityTest`、`KisAirbrushOptionDataContractTest`、`KisColorOptionDataContractTest`、`KisColorSourceOptionDataContractTest`、`KisCompositeOpOptionDataContractTest`、`KisPaintingModeOptionDataContractTest`、`KisPrecisionOptionContractTest`、`KisFilterOptionDataContractTest`、`KisTextureOptionDataIOContractTest`、`KisTextureOptionLodContractTest`は各1件成功した。`kritalibpaintop`も成功し、macOSパッケージ境界は1,736対象を確認した。`verify-quick`は45件の方針試験、責務・公開契約・テスト契約・統治・文書検査を含めて成功した。
- 残るリスク: 実行検証はmacOS・Qt 6.11.1に限る。Qt 5、Linux、Windows、Androidの実行確認はR2-G19dで扱う。残る設定画面実装は未監査である。
- 次の作業: R2-G19avを一変更化し、シャープネス設定画面を次の有限な監査単位として確定する。
