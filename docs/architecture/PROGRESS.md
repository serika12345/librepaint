...
同じ`libs/ui/nodes/kis_node_manager.h`に残る`setup()`を次の小単位とする。`kis_node_manager.cpp`に登録される54操作に
ついて、対象指定の変更なし計画、直接CMake依存、空構築閉包を監査し、操作識別子と起動先、13種のノード作成型、
5種のノード変換型と除外型、図形レイヤー限定、確認可能状態、ノード起動から分離表示への通知接続、iOSの遅延実行
方針を具体的な操作・信号対応付け器から分けて挙動契約を追加する。
