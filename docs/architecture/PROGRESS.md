...
矩形操作状態の移設をコミットした後、`libs/ui/tool/kis_tool_polyline_base.{h,cpp}`を起点に、
多角線の点列、ドラッグ座標、完了判定を`libs/tools`へ移す。輪郭表示、キャンバス更新、入力座標変換、
操作完了の接続はUI所有へ残し、既存の単一クリック、複数点、終端近接、取消し契約を観測可能な試験で
固定する。清浄な同一コミットをDarwinとx86_64 Linuxへ揃える5構成の完全一致検査はR1-G6f統合時に
実施する。
