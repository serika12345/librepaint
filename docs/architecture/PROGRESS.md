...
- ブランチ: `develop`
- 目的: R2の最適化前基準として、1:1の17.3度回転で有効描画領域、境界不透明度、色差を実測し、
  Qt平滑化へ適用する比較規則を選定する。
...
R2-G9として、8×8位置符号画像を透明な16×16描画先の中央へ置き、画像中心を静止点として17.3度
回転する。変換済み画像矩形と非透明画素の包含関係、完全不透明な内部標本、半透明境界の範囲、
逆変換した理想座標との色差を実測し、Qtや描画装置への過剰適合を避ける比較規則を先に決める。
