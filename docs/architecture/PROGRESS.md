...
- 更新日時: 2026-08-27 18:12 JST
- 状態: `planned`
- 現在の検査段階: R2-G10 タブレット入力からツール呼出しまでの契約選定
- 関連TODO: `docs/architecture/TODO.md`の「R2: 現行挙動のテスト固定」
- ブランチ: `develop`
- 目的: R2の入力契約として、Qtタブレット事象から現在ツールへ渡る位置、筆圧、傾き、回転、時刻、
  入力種別の既存観測範囲と構築境界を確認する。
...
R2-G10の実装前監査として、`KisToolProxyContractTest`の増分計画、直接依存、空構築閉包と、既存の
タブレット入力試験が観測する値を確認する。範囲が過大なら入力契約を追加する前に対象または製品責務を
分離し、最小ならQtタブレット事象からツール呼出しまでの未観測値を検査段階として定義する。
