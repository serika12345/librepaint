...
- macOSの`develop`を`540d912d`までGit履歴として`ssh nixos`上の
...
- Linux実機の永続Ninja木で`KisStrokeJobContractTest`、`kis_vec_test`、
  ...
  `KoColorModelStandardIdsContractTest`、`KisSelectionTagsContractTest`、`KisCommandIdsContractTest`だけを
  限定構築した。各CTestの単発実行と20回反復が成功し、直近の
