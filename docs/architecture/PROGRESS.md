...
- 初回契約検査は許可依存方針検査器の欠落を診断し、高速検査の統合契約は許可依存方針
  更新器の`--check`が未接続であることを診断した。
- `nix develop .#test --command ./scripts/verify-quick`: 64件の単体試験、公開ヘッダー、
  UI直下クラス、UIツールクラス、プラグイン、責務地図、許可依存方針の決定的更新検査、
...
- `nix develop .#test --command ./scripts/architecture/verify_cmake_graphs.py
  ...
  /home/masato/worktrees/librepaint-r1-g3b-verify`: macOS、Linux、iOS、Android、Windowsの
...
R1-G4aとして許可方向外の現在辺を実装箇所へ帰属させる。

1. 26射影候補と14種類の責務対を、元のCMakeターゲット辺と該当プラットフォームへ戻す。
2. 依存先公開ヘッダーと依存元ソースの直接includeを採取し、共有ターゲット内の実際の
   責務へ各利用箇所を帰属させる。
3. 確認済みの逆方向依存、循環構成要素、内部ヘッダー参照を、所有者、根拠、最大範囲、
   除去条件を持つ既存違反基準へ記録する。
4. 新規違反と基準拡大を診断し、基準縮小を同じ変更へ要求する契約試験と更新器を追加する。
5. 全検証を成功させ、R1-G4bの継続検査統合または次の最小違反分類を再開地点にする。

## R1-G3完了根拠

- 9責務から現在の所有者、公開面、クラス、プラグイン、CMake関係へ辿れる。
- 14公開接続面が目的、寿命、エラー動作を持ち、許可依存が接続面単位で定義される。
- 許可依存が常に下位層へ向かい、有向非巡回性を高速検査で確認できる。
- 15ターゲット間の現在辺が全責務候補へ射影され、許可方向とR1-G4対象を判別できる。
- 5構成の実体、責務地図、許可依存方針の一致を同時検証できる。
