...
`libs/ui/nodes/kis_node_model.h`の`flags()`を次の小単位とする。現在の木接続契約と既存モデル試験の変更なし計画、直接依存、
空構築閉包を比較し、無効索引、主列、複製列、ドロップ許可済み項目が公開するQt操作フラグを固定する。項目識別子と
ドロップ許可集合への具象アクセスは値境界へ残し、列別のフラグ合成を専用生成物へ分ける。
