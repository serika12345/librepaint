...
- 更新日時: 2026-09-21 19:04 JST
- 状態: `in_progress`
- 現在の検査段階: R2-G19ay エアブラシ設定画面依存の直接化
- 関連TODO: R2-G19a・R2-G19b・R2-G19c・R2-G19f・R2-G19g・R2-G19h・R2-G19i・R2-G19j・R2-G19k・R2-G19l・R2-G19m・R2-G19n・R2-G19o・R2-G19p・R2-G19q・R2-G19r・R2-G19s・R2-G19t・R2-G19u・R2-G19v・R2-G19w・R2-G19x・R2-G19y・R2-G19z・R2-G19aa・R2-G19ab・R2-G19ac・R2-G19ad・R2-G19ae・R2-G19af・R2-G19ag・R2-G19ah・R2-G19ai・R2-G19aj・R2-G19ak・R2-G19al・R2-G19am・R2-G19an・R2-G19ao・R2-G19ap・R2-G19aq・R2-G19ar・R2-G19as・R2-G19at・R2-G19au・R2-G19av・R2-G19aw・R2-G19ax・R2-G19ay完了、R2-G19az・R2-G19e・R2-G19d-a・R2-G19d-bは`planned`
- ブランチ: `issue-44-direct-dependencies`
- 開始コミット: `9e961f1f60`。作業開始時点の作業ツリーは変更なし。
- 目的: エアブラシ設定画面と状態モデルが、集約ライブラリーと無関係な設定モデルから偶然得るQt画面部品、画面基底型、データ、MOC処理とリンク実装に依存する状態を解消する。
- 範囲固定: `plugins/paintops/libpaintop/KisAirbrushOptionWidget.{h,cpp}`、`KisAirbrushOptionModel.{h,cpp}`と同ディレクトリーのCMake対象に限定した。エアブラシ入力、設定画面、プリセット設定、描画結果と既存CTestは変更しない。
- 調査: 初期の`misc-include-cleaner`は公開ヘッダー、画面実装、モデル公開ヘッダーで計10件の直接取込み不足を報告した。画面実装は集約ライブラリーだけに収容され、モデルは異なる設定モデルと単一のオブジェクト対象に束ねられていた。分離後の最初の構築は`kis_paintop_option.h`を探索できず、画面基底型の所有者が`kritatoolsui`であることを確認した。
- 完了: 公開ヘッダーと実装をスマートポインター、標準関数、Qt画面部品・型・メタオブジェクト、設定型、翻訳、エアブラシデータとモデル、画面接続、Lagerの所有ヘッダーへ直接接続した。モデルを`kritapaintopairbrushoptionmodelobjects`、画面を`kritapaintopairbrushoptionwidgetobjects`へ分け、画面フォームの生成も画面対象へ移した。モデルはQt Core、エアブラシデータ、Lagerを、画面はQt Core・Widgets、エアブラシデータとモデル、画面基底型、Lagerを公開利用要件、翻訳と画面接続を実装専用依存として明示する。MOCは各専用対象で実行し、集約ライブラリーは両オブジェクトを一度だけ取り込む。既存の`KisAirbrushOptionDataContractTest`が保存値の読み書きを維持する。
- 検証: `run-shared-test-env`経由で専用画面対象と`kritalibpaintop`を成功させ、macOSパッケージ境界は1,741対象を確認した。公開ヘッダーと実装の`misc-include-cleaner`は警告0件である。Ninja閉包は専用画面対象1,259工程、最終ライブラリー2,142工程である。既存の`KisAirbrushOptionDataContractTest`は1件成功した。`verify-quick`は45件の方針試験、責務・公開契約・テスト契約・統治・文書検査を含めて成功した。
- 残るリスク: 実行検証はmacOS・Qt 6.11.1に限る。Qt 5、Linux、Windows、Androidの実行確認はR2-G19dで扱う。残る設定画面実装は未監査である。
- 次の作業: R2-G19ayを一変更化し、合成方法設定画面を次の有限な監査単位として確定する。
