...
- 関連TODO: R1-G1からR1-G8、R2-G19bm、R2-G19d-0、R2-G19d-a、R2-G19d-cからR2-G19d-g、R2-G19d-iからR2-G19d-kを完了。R2-G19d-hを実機受入れ待ちとし、独立したR2-G19d-bは`planned`のまま維持する
- 追跡: キャンバス専用表示の上部ブラシ一覧から別ブラシを選ぶとApple Pencilダブルタップが以後反応しなくなる実機報告
- ブランチ: `develop`
- 開始コミット: `ca65d3ef38003cd6a0ecf4eb6652f300df4e1b54`。開始時の作業ツリーは変更なしである。
- 目的: Apple Pencilのダブルタップ受付をLibrePaintのメインUIKitウィンドウが継続して所有し、補助ウィンドウの表示後も既存の消しゴム・直前プリセット操作へ接続する。
- 範囲固定: 起点`krita/KisIOSPencilInteraction.mm`のUIKit登録先選択だけを変更する。`krita/main.cc`の既存アクション接続、Pencil描画、ブラシ選択、キャンバス専用表示、他OSの動作を維持する。
- 原因と動的診断: 実機コンソールでは起動時に`UIPencilInteraction`がLibrePaintの`QUIWindow`へ登録され、ダブルタップが`eraser_preset_action`へ到達した。上部ブラシ一覧の操作後、UIKitが画面外キーボード用`UIRemoteKeyboardWindow`を可視化すると、全ウィンドウの可視化通知を追従していた登録処理が対話オブジェクトを同ウィンドウへ移し、その後のダブルタップ通知が停止した。子の`QUIMetalView`へ固定した実機比較では登録後も通知を受信しなかったため、受信契約の所有先はメイン`QUIWindow`である。
- 実装結果: 起動時に`QWindow::winId()`から渡されたメインネイティブビューを保持し、そのビューが属する`QUIWindow`をUIKit接続後に一度だけ確定する。可視化、キー化、前景復帰の通知は同じ確定済みウィンドウへの登録確認にだけ用い、通知元の補助ウィンドウを候補にしない。全scene・全windowの探索と接続先の移動を撤去した。
- 契約: UIKit scene接続時期と物理Apple Pencil入力は自動化されたnative試験で再現できないため、iOS arm64実コンパイル、最終リンク、物理端末の接続ログ、ブラシ選択前後とキャンバス専用表示解除後のダブルタップを実機契約とする。
- 検査状態: iOS arm64は`KisIOSPencilInteraction.mm`を実コンパイルして`LibrePaint.app/LibrePaint`へ最終リンクし、保持7群253ファイル・除外1群・未分類0件の静的依存資源監査に成功した。ビルド`20260926055407`のIPAは27ディレクトリー、データ207ファイル、実行形式1ファイルの内容・権限監査に成功し、iPad ProへAltStoreで更新した。コンソールはメインウィンドウへの登録を一度だけ記録し、通常表示、キャンバス専用表示、ブラシ選択後、通常表示への復帰を通じて8回の`eraser_preset_action`到達を記録した。利用者は全手順を問題なしとして受け入れた。
- 残るリスク: 現在のiPad製品は単一のQtメインウィンドウをプロセス寿命で所有する。将来複数sceneまたはメイン`QWindow`の再生成を導入する場合は、全ウィンドウ探索ではなくQt側の明示的な所有移行通知を追加する。
- 次の作業: R2-G19d-hに残る通常ピンチ、回転、中断、誤発動の実機受入れ結果を確定する。

## 直前の完了記録: R2-G19d-j iPad上端ツールバーと全体表示領域契約

- 更新日時: 2026-09-26 13:26 JST
- 状態: `complete`
- 現在の検査段階: R2-G19d-j iPad上端ツールバーと全体表示領域契約
- 関連TODO: R1-G1からR1-G8、R2-G19bm、R2-G19d-0、R2-G19d-a、R2-G19d-cからR2-G19d-g、R2-G19d-i、R2-G19d-jを完了。R2-G19d-hを実機受入れ待ちとし、独立したR2-G19d-bは`planned`のまま維持する
- 追跡: 利用者によるProcreate相当の上端ツールバー配置とクイックピンチ表示領域要求
- ブランチ: `develop`
- 開始コミット: `e0e9779faf29aa620df470509d8dd2a4a5ac57c9`。このコミットが利用者の受入れ済み状態への切り戻し点である。
- 目的: iPadの上部描画操作バーを画面上端へ固定し、クイックピンチ時はその下の可視領域へキャンバス全体を配置する。
- 範囲固定: 起点`plugins/extensions/iostouchui/iostouchui.cpp`の上部バー配置から、行先`libs/input/KisQuickPinchRecognizer.{h,cpp}`の全体表示幾何と`libs/ui/actions/input/kis_zoom_and_rotate_action.cpp`のクイックピンチ変換へ接続する。確定済みの文書タイトルバー制御、通常ピンチ、Androidとデスクトップの表示領域を維持する。
- 契約と初期診断: 利用者の参照画像では上部バーが画面上端と左右端へ接し、キャンバス全体はバー下の領域で中央揃えされる。現在の上部バーはキャンバスから全周に8論理画素離れ、全体表示は描画操作バーと重なるキャンバス全体を基準に倍率と中心を決めている。
- 対象構築基準: `KisQuickPinchRecognizerContractTest`は認識・幾何実装と試験の5構築工程だけを持つ。`kritaiostouchui`は既存の入力UIと共有UI依存内で接続し、新しい対象を追加しない。
- 実装結果: 上部バーの外周余白と角丸を外し、キャンバスと安全領域が交わる矩形の上端・左右端へ固定した。タッチUIはバー下の矩形をキャンバス座標で公開し、非表示、文書切替、キャンバス専用表示の解除でその指定を消去する。クイックピンチは公開された矩形の幅と高さから目標倍率を決め、同じ矩形の中心へ画像中心を移す。指定のないAndroidとデスクトップは従来どおりキャンバス全体を使う。
- 検査状態: 上部に60論理画素の重なりがある1000×800の表示で、余白20を含む1000×1000の画像が倍率0.7、表示中心(500, 430)となる契約は、新しい目標計算がない初期状態でコンパイル失敗し、実装後の対象試験と20回反復に成功した。`kritaapplicationui`のnative再構築とmacOSパッケージ境界1757対象、`verify-quick` 57/57件に成功した。iOS arm64は14工程で入力幾何、共有UI、iPadタッチUIを実コンパイルして最終リンクし、保持7群253ファイル・除外1群・未分類0件の静的依存資源監査に成功した。ビルド`20260926042055`をiPad ProへAltStoreで更新し、署名済み`local.librepaint.ipad.PUDY4GHY3Y`の起動と活動状態を確認した。利用者は実機上の上端バー配置とバー下領域へのクイックピンチ全体表示を受け入れた。
- 残るリスク: iPadの対象表示は実機で受け入れ済みである。共有クイックピンチのAndroid実機操作はR5の端末検査で扱う。
- 次の作業: R2-G19d-hに残る通常ピンチ、回転、中断、誤発動の実機受入れ結果を確定する。

## 直前の完了記録: R2-G19d-i iPadキャンバス専用文書タイトルバー契約

- 更新日時: 2026-09-26 13:10 JST
- 状態: `complete`
- 現在の検査段階: R2-G19d-i iPadキャンバス専用文書タイトルバー契約
- 関連TODO: R1-G1からR1-G8、R2-G19bm、R2-G19d-0、R2-G19d-a、R2-G19d-cからR2-G19d-g、R2-G19d-iを完了。R2-G19d-hを実機受入れ待ちとし、独立したR2-G19d-bは`planned`のまま維持する
- 追跡: 利用者によるフルスクリーン上部の文書タイトルバー削除要求
- ブランチ: `develop`
- 開始コミット: `1262786f1b0c88dd092acf9b88be8a61e1c58200`。開始時の作業ツリーは変更なしである。
- 目的: iPadのキャンバス専用表示で文書名のタブバーを隠し、キャンバスと描画操作へ使用できる縦領域を広げる。
- 範囲固定: 起点`libs/application/ui/workspace/KisViewManager.cpp`のキャンバス専用状態切替から、行先`libs/application/ui/workspace/KisMainWindow.{h,cpp}`の文書タブバー表示所有へ接続する。iOSのキャンバス専用表示だけを対象とし、上部の描画操作バー、通常表示、デスクトップの文書タブ、複数文書状態を維持する。
- 契約と初期診断: 物理iPadのキャンバス専用表示で、文書名を表示するタブバーが画面上部に残ることを利用者が確認した。ビルド`20260926034310`ではタブを非表示にして文書名を消した一方、Qtの`QMdiArea`がタブバー高の表示領域余白を保持したため、画面上端の帯が残った。続く`20260926034839`は余白を同期的にゼロへ変更したが、Qtのタブ領域再計算と相互に寸法変更を呼び、フルスクリーン切替でクラッシュした。端末から回収した2件の`.ips`はいずれも`KisMdiArea::resizeEvent()`と`QMdiAreaPrivate::updateTabBarGeometry()`間の約2700段の再帰によるスタック枯渇を示した。再入防止を加えた`20260926035351`もOpenGLキャンバスの再描画と寸法変更を反復し、10秒の画面更新監視時間を超えてiPadOSに強制終了された。余白操作を撤去した`20260926040019`ではタブバーの代わりに`SubWindowView`の文書タイトルと最小化・復元・閉じるボタンを含む枠が上端へ現れた。削除要素の不在だけを固定する試験は利用者指示により追加せず、既存のキャンバス専用状態遷移、製品構築、実機上の表示と通常表示への復帰を契約とする。
- 対象構築基準: 既存の`kritaapplicationui`がキャンバス専用状態と文書タブバーを所有するため、新しい対象と依存は追加しない。
- 実装結果: 表示領域余白へ直接介入する実装を撤去した。キャンバス専用表示へ入る際は元の文書表示方式と現在文書のウィンドウ属性を保持し、Qtの公開された`SubWindowView`へ切り替えてタブバーと予約領域をQt自身に撤去させる。現在文書には枠なし属性を付けて最大化し、文書タイトルとウィンドウボタンを含む枠全体を表示対象から外す。解除時は元の属性と`TabbedView`を復元し、複数文書を最大化状態へ戻してタブの操作設定を再適用する。上部のiPad描画操作バーは維持する。
- 検証状態: 枠なし文書表示版の`kritaapplicationui` native再構築とmacOSパッケージ境界1757対象に成功した。iOS arm64は変更箇所を実コンパイルして最終リンクし、保持7群253ファイル・除外1群・未分類0件の静的依存資源監査に成功した。ビルド番号`20260926040612`のIPAは27ディレクトリー、データ207ファイル、実行形式1ファイルの内容・権限監査に成功し、iPad ProへAltStoreで更新して署名済み`local.librepaint.ipad.PUDY4GHY3Y`の起動を確認した。文書更新後の`verify-quick` 57/57件にも成功し、導入後の新しいクラッシュ記録はない。利用者は同ビルドで文書タイトルとウィンドウボタンを含む上端の枠が消え、フルスクリーン切替が正常であることを受け入れた。
- 残るリスク: 共有主画面の変更はiOS条件内に限定した。Androidの複数指操作とデスクトップの文書表示は各プラットフォームの既存検査で維持する。
- 次の作業: iPad描画操作バーを画面上端へ固定し、その下の利用可能領域をクイックピンチ全体表示の対象とする独立した検査段階を開始する。

## 継続中記録: R2-G19d-h モバイルクイックピンチ全体表示契約

- 更新日時: 2026-09-26 12:33 JST
- 状態: `in_progress`
- 現在の検査段階: R2-G19d-h モバイルクイックピンチ全体表示契約
- 関連TODO: R1-G1からR1-G8、R2-G19bm、R2-G19d-0、R2-G19d-a、R2-G19d-cからR2-G19d-gを完了。R2-G19d-hを実装・検証中とし、独立したR2-G19d-bは`planned`のまま維持する
- 追跡: 利用者によるProcreate相当のクイックピンチ全体表示要求
- ブランチ: `develop`
- 開始コミット: `4bdd25e9cb926e466e77286d40a3fcd3ac8c70ad`。開始時の作業ツリーは変更なしであり、`develop`と`origin/develop`は一致している。
- 目的: 二指で素早く内向きへピンチして指を離す一操作で、キャンバスを最寄りの90度単位へ整列して表示領域へ全体表示する。
- 範囲固定: 起点`libs/ui/actions/input/kis_zoom_and_rotate_action.{h,cpp}`の共有二指操作から、行先`libs/input/KisQuickPinchRecognizer.{h,cpp}`の時間・距離・回転判定と最寄りの90度単位の向き判定を経て既存の回転・`ZOOM_PAGE`操作へ接続する。入力プロファイル、通常速度の拡大縮小・回転、Pencil描画、表示倍率計算、iOSタッチUIパネルは維持する。
- 対象構築基準: 既存`KisApplicationInputActionsTest`の空構築閉包は2032工程で判定だけを検査するには過大であった。Qt Coreだけへ依存する`KisQuickPinchRecognizerContractTest`と製品オブジェクト対象を追加し、空構築閉包を5工程へ限定した。共有UIは既存の`kritaapplicationui`から`kritainput`への直接依存を使い、新しい外部依存を追加しない。
- 契約と初期診断: 速い内向き操作、遅い操作、小さい縮小、外向き操作、回転主体、縮小後の反転、取消しを値で検査する。最初の実行は未実装の`KisQuickPinchRecognizer.h`が存在せずコンパイルに失敗した。ビルド番号`20260926030357`の実機確認では斜めのキャンバスにクイックピンチを行うと倍率だけが変わり、回転が残る契約違反を確認した。続く`20260926031010`への利用者フィードバックでは、縦向きに近い回転も0度へ戻すのではなく縦方向へ収める必要が判明した。0度・90度・180度・反時計回り・周回角度の境界試験は、向き判定が未実装のため`snappedCanvasRotation`不在のコンパイル診断で最初に失敗した。`20260926031654`へのフィードバックでは全体表示が一瞬で切り替わるため、連続的な遷移が必要と判明した。縦横別の目標倍率と中間変換の契約は`KisQuickPinchTransform`不在のコンパイル診断で最初に失敗した。
- 実装結果: 350ミリ秒以内、開始距離の65%以下、回転10度以内、解放前の戻り8%以内をクイックピンチとする。二指拡大縮小・回転は更新中の既存処理を維持し、二指の解放時に現在角度を45度境界で最寄りの90度単位へ整列した全体表示を求める。開始時の角度・倍率・画像中心位置から目標状態までを180ミリ秒の`OutCubic`曲線で同時に補間し、倍率には幾何補間を用いる。完了時は計算済みの目標倍率と目標中心を定数倍率の表示変換として適用し、新しい指操作ではアニメーションを現在位置で中断する。三指への変更、取消し、遅い操作では判定を破棄する。Qt 6では各接触点の押下時刻を使用し、未提供時とQt 5では単調時計へ戻る。
- 検証状態: `KisQuickPinchRecognizerContractTest`は向き境界、縦横別目標倍率、角度・倍率・中心位置の中間変換を含む対象実行と20回反復に成功した。`kritaapplicationui`のnative再構築、既存の`kis_coordinates_converter_test`、`verify-quick` 57/57件も成功した。アニメーション追加後の完全native検査は新規契約を含む882/882件すべて成功し、前回の`KisSafeDocumentLoaderTest::testFileLost()`並列競合も再現しなかった。iOS arm64はアニメーション値処理、入力ライブラリー、共有UIを実コンパイルして`LibrePaint.app/LibrePaint`へリンクし、保持7群253ファイル・除外1群・未分類0件の静的依存資源監査に成功した。ビルド番号`20260926032639`のIPAは27ディレクトリー、データ207ファイル、実行形式1ファイルの内容・権限監査に成功した。同ビルドをiPad Pro（11-inch）へAltStoreで更新し、署名済み`local.librepaint.ipad.PUDY4GHY3Y`の起動、データベース初期化、活動状態到達、既存文書の再読込みを確認した。
- 残るリスク: アニメーション版は物理iPadへ導入済みだが、180ミリ秒の遷移時間と減速曲線、縦横方向の表示、途中で新しい指を置いた場合の中断感は利用者操作で未確認である。350ミリ秒、距離65%、回転10度、戻り8%の判定値が通常の倍率微調整とクイックピンチを識別できるかも実機受入れを要する。共有実装のAndroid実機複数指操作はR5の端末検査で扱う。
- 次の作業: 導入済みビルド`20260926032639`で横向き・縦向きの遷移、通常ピンチ、回転、途中中断、取消し、誤発動を受け入れ確認する。実機結果に基づいて遷移時間、曲線、判定値を調整し、受入れ成功後にR2-G19d-hを`complete`へ更新する。

## 直前の完了記録: R2-G19d-g iPad全画面パネル配置契約

- 更新日時: 2026-09-26 01:20 JST
- 状態: `complete`
- 現在の検査段階: R2-G19d-g iPad全画面パネル配置契約
- 関連TODO: R1-G1からR1-G8、R2-G19bm、R2-G19d-0、R2-G19d-a、R2-G19d-cからR2-G19d-gを完了。独立したR2-G19d-bは`planned`のまま維持する
- 追跡: 利用者による全画面ブラシ一覧の横長・疎な配置と、色選択が左上に出る実機報告
- ブランチ: `develop`
- 開始コミット: `cd8e2512dfe6e8d19249ebf801218ee8a6015893`。開始時の作業ツリーは変更なしであり、`develop`と`origin/develop`は一致している。
- 目的: iPadのキャンバスのみ表示で、ブラシ一覧を縦方向へ密に配置し、色選択を上部の色ボタンに対応する安全領域内へ表示する。
- 範囲固定: 起点`plugins/extensions/iostouchui/iostouchui.cpp`のブラシ・色パネルと所有CMake対象、iOS設計・配備・進捗文書に限定する。デスクトップのプリセット表示設定と色ダイアログ、資源・色の選択結果、描画処理は維持する。
- 原因と初期診断: ブラシパネルは固定寸法を持つ汎用ポップアップへ応答配置を有効にし、高さ判定によって横一列のアイコン帯へ切り替わった後も外側パネルだけが620×650論理画素まで拡大していた。色ボタンはデスクトップ用`chooseForegroundColor`操作を呼び、独立ダイアログの座標をiPadOSの表示領域へ指定していなかったため左上へ出ていた。
- 対象構築基準: 変更前のiOS増分計画は0工程である。iOS限定の`kritaiostouchui`にはnative実行対象がないため、表示契約はiOS arm64の実コンパイル、最終リンク、物理端末操作で検査する。色選択画面の具体所有者`kritawidgets`を直接依存へ追加する。
- 実装結果: ブラシパネルは`KisPresetChooser`の詳細表示を直接所有し、名前付き行を上から下へ並べて既存のタッチ慣性スクロールとタグ絞込みを使う。幅は画面の約40%かつ最大480論理画素とした。色選択は同プラグインが子画面として所有し、上部バー直下の右端へ安全領域と利用可能高さに収める。ブラシ、レイヤー、色の表示は相互排他的で、回転・寸法変更時に同じ配置処理を通る。前景色の即時反映、OK、Cancel、表示色変換は既存画面と資源提供者へ接続した。
- 検証状態: iOS増分計画は再構成後7工程で、`iostouchui.cpp`の実コンパイル、静的プラグイン作成、`LibrePaint.app/LibrePaint`の最終リンクに成功した。最終構築は5工程で同じ最終リンクに成功し、最終iOS増分計画は0工程である。静的依存資源は保持7群、253ファイル、除外1群、未分類0件である。ビルド番号`20260925161738`のIPAは27ディレクトリー、データ207ファイル、実行形式1ファイルの権限・内容監査に成功した。`verify-quick`は57/57件、方針、文書、リンク、図の検査に成功した。
- 実機受入れ: iPad Pro（11-inch、iPadOS 26.6）へAltStoreで同じバンドル識別子`local.librepaint.ipad.PUDY4GHY3Y`を更新し、既存データを保持して起動した。起動ログはデータベース初期化と活動状態到達を記録し、新しい致命的診断はない。利用者がブラシ一覧と色選択パネルの修正結果を確認し、問題なしとして受け入れた。
- 残るリスク: 他寸法のiPad、Split View、Stage Managerの狭幅配置は今回の単一端末受入れに含めておらず、R5の端末別UI検査で扱う。
- 次の作業: 変更をレビューし、R5でSplit ViewとStage Managerを含む複数画面寸法のタッチUI配置契約を追加する。独立したR2-G19d-bは`planned`のまま維持する。

## 過去の完了記録: R2-G19d-f Apple Pencilダブルタップ登録契約
...
## 過去の完了記録: R2-G19d-e タッチ操作による入力割当削除契約
...
- 追跡チケット: [Issue #40 iPadでLibrePaintフォルダ外に文書を保存できない](https://github.com/serika12345/librepaint/issues/40)、[PR #54 Allow iPad saves through Files document providers](https://github.com/serika12345/librepaint/pull/54)
- ブランチ: `issue-40-ios-file-provider-save`
- 開始コミット: `fa32614610`。開始時の作業ツリーは変更なしであり、`develop`と`origin/develop`は一致している。
- 目的: iPadOSの文書ピッカーが許可した単一ファイルへの保存が、隣接バックアップと同一ディレクトリーの安全保存用一時ファイルを要求して失敗する問題を解消する。
- 範囲固定: `libs/document/files/`の文書ファイル交換・バックアップ・自動保存方針、`libs/impex/ui/KisImportExportManager.cpp`のiOS書出し接続、対応するCMakeと契約試験、iOS検証文書に限定する。保存形式、画像内容、公開操作、他OSの安全保存は維持する。
- 原因: `UIDocumentPickerViewController`は選択した出力ファイルのセキュリティー保護付きURLを返すが、親ディレクトリーの書込み権限は与えない。現行は空の出力ファイルを既存文書と判定して隣接バックアップを作り、バックアップ無効時も`QSaveFile`が隣接一時ファイルを要求するため失敗する。
- 対象構築基準: `kis_document_files_test`の変更前増分計画は18工程。`kritadocumentfiles`の既存直接依存はQt Core、`kritaglobal`、`kritaresourcestorage`であり、新しい依存対象は追加しない。責務をimport-exportへ確定した後の新規`kis_import_export_file_transfer_test`は空構築命令閉包7件であり、近接する`kis_import_export_error_code_test`の78件より狭い。製品オブジェクトはQt Core、試験は同オブジェクトとQt Testだけへ直接接続する。
- 構造境界: 起点`libs/impex/ui/KisImportExportManager.cpp`のiOS向け`QSaveFile`経路を、行先`libs/impex/KisImportExportFileTransfer.{h,cpp}`の選択ファイル直接書込みへ移した。書出しフィルターはアプリ一時領域で完成物を生成・検証し、転送処理は選択ファイルを削除せず同じファイルハンドルから内容を置き換える。検証または反映の失敗時は完成物を残し、その場所を診断へ出力する。
...
- 関連TODO: R1-G1からR1-G8およびR2-G19bmまで完了、R2-G19d-aは`planned`
- ブランチ: `develop`
- 開始コミット: `b85033aced2ad5b25c285518f498de33117c132d`。実装は`183107a06c`、`a10a3426bb`、`f03078ea22`、`58a24eab42`、`9abd27a14e`としてコミット済みである。
- 目的: 高頻度利用の公開ヘッダーが、識別子の格納、曲線のスプライン計算、資源表示名の翻訳という私有実装を全利用元へ到達させる問題を解消し、実装変更の再コンパイル波及を実際の所有者へ局所化する。
- 範囲固定: R1-G8aの測定基準から、`KoID`の翻訳格納、`KisCubicCurve`の計算実装、`KisResourceTypes`の翻訳型、それらの5構成統合に限定した。公開API、ABI、保存形式、描画算法、新しい抽象とCMake対象は変更していない。
- 構造移動: 起点`libs/global/KoID.h`の`TranslatedString`、`StorageType`、`KoIDPrivate`完全定義を、行先`libs/global/KoID.cpp`へ移した。起点`libs/image/kis_cubic_curve.h`の`kis_cubic_curve_spline.h`取込みを、行先`libs/image/kis_cubic_curve.cpp`へ移した。起点`libs/resources/KisResourceTypes.h`の`KLocalizedString`完全定義を、行先`libs/resources/KisResourceTypes.cpp`と`libs/resources/tests/KisResourceTypesContractTest.cpp`、および実利用元252ファイルの所有ヘッダーへ移した。
- 利用要件: `libs/image/CMakeLists.txt`の`Eigen3::Eigen`と`libs/resources/CMakeLists.txt`の`KF${KF_MAJOR}::I18n`を各所有対象の私有利用要件にし、契約試験は実際に使う所有対象へだけ直接接続した。翻訳マクロを使う246ファイルと翻訳値のメソッドを使う6ファイルはKDE翻訳ヘッダーを直接取り込む。
- 空構築境界: Linuxの空構築で公開利用元が輸出ヘッダーを解決できない問題を確認した。起点`build/<preset>/libs/ui/kritaui_export.h`から、行先`build/<preset>/libs/application/kritaui_export.h`を`kritaapplicationui`の正式な生成位置とし、分割UIオブジェクト向けの`libs/ui`にも同一ヘッダーを同期する。
- 変更波及: `KisLazyStorage.h`経由の推移的コンパイルを2151工程から7工程、生成を252件から0件、直接リンク対象を571件から4件へ縮小した。`kis_cubic_curve_spline.h`経由は直接取込み3ファイルから2ファイル、推移的コンパイル1088工程から2工程、影響392対象から2対象、直接リンク331対象から1対象へ縮小し、必要な再リンク閉包489件は維持した。
- 翻訳境界: `KisResourceTypes.h`経由の翻訳実装の再コンパイル経路を1355工程から0工程にし、252個の実利用元が翻訳所有者を明示する。資源キー契約自体の174直接利用元、467直接リンク対象、655再リンク閉包、43所有対象利用元は必要な波及として維持した。
- 契約維持: `KoID`の識別子、遅延翻訳、値意味論、比較、デバッグ出力、`KisCubicCurve`の点、評価、転送表、直列化、資源種別の保存キーと表示名を既存契約試験と互換試験で維持した。
- macOS検証: 最終状態の`./scripts/verify`はパッケージ境界1752対象、スクリプト試験50件、方針・文書検査、CTest 879/879件を426.24秒で成功し、`verify: OK`となった。
- iOS検証: 最終変更の再構成11工程で`LibrePaint.app`をリンクし、静的依存資源監査は7保持群、253ファイル（画像248）、除外1群、未分類0件で成功した。パッケージ境界は1629対象、直後の増分計画は0工程である。
- Linux検証: x86_64 Linuxで1766対象の境界を検査し、同じCMake File APIグラフから取得した233製品対象を構築して`krita`実行ファイルをリンクした。`build-incremental linux plan krita`は作業なしとなった。
- Windows検証: x86_64 LinuxホストからWindowsの1670対象の境界を検査し、4155工程で`krita.exe`、`krita.com`、全共有部品をリンクした。増分計画の残りは翻訳生成2工程と既存のWebP出力再リンク1工程である。
- Android Qt 5検証: x86_64 LinuxホストからAndroidの1642対象の境界を検査し、3903工程で`libkrita_arm64-v8a.so`と全製品部品をリンクした。増分計画の残りは既存の翻訳生成2工程である。
- 構築環境: 遠隔ホストの共有試験実行環境を一度作成したとき、回収可能な`*-source`と`*-librepaint-source`は2経路・1,133,707,640バイトから4経路・2,265,998,344バイトへ増加した。以後のローカルflake評価を停止し、主worktreeのキャッシュ済み共有試験実行環境をLinux、Windows、Androidで再利用した。
- 保持成果物: 遠隔検証worktree`/home/masato/Documents/librepaint-r1-g8`、各構成の構築木、主worktreeの共有実行環境とコンパイラーキャッシュは、R2-G19d-aの対象CTest構築と実行に再利用できる状態で保持した。
- 残るリスク: macOSで全CTest、iOSで製品構築と監査、Linux、Windows、Android Qt 5でコンパイルと最終リンクを検査した。WindowsとAndroidの端末上実行、および各OSの移植済み契約CTestの実行はR2-G19d-aの範囲である。
- 次の作業: R2-G19d-aでQt 5、Linux、Windows、Androidの実行環境と構築閉包を確定し、`KisCurveOptionDataTest`、`KisCurveOptionModelTest`、`KisStandardOptionDataCompatibilityTest`、`KisKritaSensorPackCompatibilityTest`、`KisMirrorOptionDataCompatibilityTest`、`KisBrushPresetDynamicsCompatibilityTest`、`TestTagFilterResourceProxyModel`を実行する。
