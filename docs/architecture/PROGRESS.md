...
同じ`libs/ui/nodes/kis_node_manager.h`に残る12 APIのうち、画素ノードと図形レイヤーの書き出しを担う2 APIを次の
小単位とする。`kis_node_manager.cpp`の`saveNodeAsImage()`と`saveVectorLayerAsImage()`について、対象指定の変更なし
計画、直接CMake依存、空構築閉包を監査し、活動ノードと画素投影の前提、画像とノードを合わせた保存範囲、解像度・
不透明度、図形レイヤー限定、SVG保存寸法とZ順、取消し・失敗通知を具体的なファイルダイアログ、文書書き出し、
SVGライターから分けて挙動契約を追加する。
