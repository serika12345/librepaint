...
- macOSの`develop`を`27ac0ea6`までGit履歴として`ssh nixos`上の
...
- Linux実機の永続Ninja木で`KisStrokeJobContractTest`、`kis_vec_test`、
  ...
  `KritaContainerUtilsContractTest`、`TestKoIntegerMaths`、`KoColorSpaceConstantsContractTest`だけを
  限定構築した。
  ...
  10入力、コンテナー補助試験、整数演算試験、不透明度端点試験は各4工程・10入力である。全ネイティブ
  検証は実行していない。
