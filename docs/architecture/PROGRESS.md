...
`libs/ui/nodes/kis_node_model.h`の`setDummiesFacade()`を次の小単位とする。現在の寿命契約と既存モデル試験の変更なし計画、
直接依存、空構築閉包を比較し、ダミー木の切替時に行うリセット、信号接続、表示方式接続、索引変換器再生成、モデルリセットの
順序と、空ファサードへ戻す状態を固定する。具象接続の閉包が責務を越える場合は、接続計画と具象実行の境界を先に分ける。
