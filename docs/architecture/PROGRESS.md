...
`libs/ui/nodes/kis_node_model.h`の`canDropMimeData()`を次の小単位とする。現在の項目操作契約と既存モデル試験の変更なし計画、
直接依存、空構築閉包を比較し、有効な親項目上へのドロップを常に許可する規約と、ルートへのドロップをQt基底判定へ委譲する
規約を固定する。Qtモデルの値判断だけで閉じる場合は専用生成物へ分ける。
