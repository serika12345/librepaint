...
- 更新日時: 2026-08-27 18:56 JST
- 状態: `planned`
- 現在の検査段階: R2-G14 既定画素ブラシの筆圧応答契約
- 関連TODO: `docs/architecture/TODO.md`の「R2: 現行挙動のテスト固定」
- ブランチ: `develop`
- 目的: R2-G13bの描画条件から筆圧だけを0.5へ変え、既定画素ブラシの寸法・不透明度応答と補間後の
  画素結果を固定する。
...
R2-G14の実装前監査として、`FreehandStrokeContractTest`の変更なし計画、直接依存、1,107工程の
空構築閉包を再確認する。固定入力生成処理へ筆圧を値として渡し、筆圧1.0の契約を維持したまま、
同じ2入力点を筆圧0.5で描くレイヤー画素、投影、正確な描画領域の最初の診断を採取する。
