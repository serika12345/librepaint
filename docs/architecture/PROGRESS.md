...
同じ`libs/ui/nodes/kis_node_manager.h`に残る25 APIのうち、活動ノードのアルファをマスクへ分離し、透明度マスクを
親レイヤーまたは保存画像へ統合する3 APIを次の小単位とする。`kis_node_manager.cpp`のアルファ操作入口について、
対象指定の変更なし計画、直接CMake依存、空構築閉包を監査し、変更可否と編集可能画素の前提、マスク名、親への
書込みと保存への分岐を具体的なレイヤー・ファイル処理から分けて挙動契約を追加する。
