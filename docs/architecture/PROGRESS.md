...
`libs/resources/ui/KisResourceItemListView.h`の資源項目一覧13 APIについて、分離済み運動スクロールと
アイコン説明表示を利用する清浄時構築閉包と直接依存を監査し、3表示方式、項目寸法、厳格選択、
説明表示設定、選択・クリック・文脈メニュー通知、未送出の寸法通知を最小契約で分類する。
