...
現在は`kritacanvas`の17件、`kritaimage`の332件、`kritaimpex`の11件、
`kritaimpexui`の23件、`kritaui`の249件を全件記録し、
...
宣言された現存クラスと構造体92件を、宣言、実装単位、所有ターゲット、5構成、責務領域へ
接続する。90件は実装単位を持ち、2件は宣言側で完結する。責務領域はアプリケーション調整、
...
R1-G6e後半は[文書ドメイン分離計画](document-domain-separation-plan.md)に従い、状態の
追加抽出ではなく利用事例単位の分離を行う。`kritadocument`は文書状態と判断、
`kritadocumentusecases`は保存、回復、読込の利用事例と副作用接続面、
`kritadocumentpersistence`はファイル、設定、時刻、形式変換、背景処理の具体アダプター、
`kritadocumentui`はダイアログ、状態表示、Qt Widgets用操作を所有する。UIから利用事例、
利用事例からドメインへ依存し、具体アダプターは利用事例所有の接続面を実装する。

最初の検査段階は、文書と取り消し履歴の接続および履歴表示を`kritadocumentui`へ移し、
`kritadocument`のリンク閉包からQt GuiとQt Widgetsを除去する。続いて回復用自動保存を
最初の縦方向利用事例とし、
要求、判断、実行指示、副作用結果、UI通知を一つの契約で分離する。各新規接続面は同じ段階で
製品アダプターと決定的な試験用実装を持ち、汎用サービス探索器や共通利用事例基底クラスは
文書境界へ導入しない。
