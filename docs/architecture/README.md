...
| --- | --- | ---: | --- |
| アプリケーション調整 | 描画 | 10 | `kritaui`内の起動登録、設定、公開作業空間型から画像、ブラシ、色処理 |
...
一意に帰属する。この規則で10件の直接includeが現在の確認済み基準になる。内訳は、公開
ヘッダーが画像型を宣言する2件、起動時の具体所有者接続5件、設定とセッション状態3件である。
...
画像待機ダイアログは待機通知への表示登録を、アイドル監視はメモリー統計更新との接続を所有する。
...
登録する。`KisApplication`は同じ登録順序で各所有者を呼び出し、フィルター、生成器、ペイント
操作、メタデータのプラグインを`KoPluginLoader`のサービス種別から読み込む。起動側に残る3件は
3所有者の登録入口である。
...
1種類10件の逆方向依存、2ヘッダー2件の内部参照を正本へ照合し、最終状態のゼロ上限を確認する。
...
- `libs/ui/workspace/KisViewManager.cpp`のキャンバス資源変換器、更新仲介、活動資源依存の
