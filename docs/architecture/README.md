...
`kritaimpexui`の23件、`kritapainting`の19件、`kritatools`の19件、
`kritaui`の225件を全件記録し、
...
一意に帰属する。この規則で103件の直接includeが現在の確認済み基準になる。R1-G6aは、
...
1件除去し、入力から描画への上限を24件へ縮小した。

共有ターゲットの保守的射影から生じた残り12責務対は、構造違反基準が実際の
...
初期上限と段階別上限は319、272、177、177、104、104、104、75、0と縮小する。内部ヘッダーの
...
次はR1-G6fの完了監査で残るUIツールクラスと依存を照合し、完了条件を満たした後、
`libs/ui/input`と`libs/ui/tool`の入力解釈を`libs/input`の`kritainput`へ移すR1-G6gを開始する。
...
9責務と5構成の現行ターゲット、3種類103件の逆方向依存、4ヘッダー7件のUI内部参照を
