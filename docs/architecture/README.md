...
宣言された現存クラスと構造体82件を、宣言、実装単位、所有ターゲット、5構成、責務領域へ
接続する。80件は実装単位を持ち、2件は宣言側で完結する。責務領域はアプリケーション調整、
キャンバス・表示、文書状態、ツール呼出し、ウィンドウ・作業空間の5種類である。
描画設定表示と入出力として所有先へ移動したクラスは更新時に台帳から除く。

この台帳は`libs/ui`直下の公開クラスを全件対象とする。
[文書境界評価](document-boundary-assessment.json)は、文書状態に分類された残る22クラスと
...
文書表示の集約で`KoDocumentInfo`と`KoDocumentInfoDlg`がUI分類から外れ、現在は22クラスである。
...
ファイル移動を先行させず、次はR1-G6fで具体的なツール命令とその表示配線を分ける。
