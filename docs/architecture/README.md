...
この文書は、変更内容から調査対象を絞り、LibrePaintの主要な設計境界と実行経路を把握するための入口です。クラス単位の網羅的な説明ではなく、次の判断に必要な情報をまとめています。
...
図は責務と主要な実行経路を示します。完全な`target_link_libraries`グラフではありません。実際のリンク境界は各ディレクトリーの`CMakeLists.txt`を正本とします。
...
- `krita/`はプロセスの入口、アプリケーション資産、OSライフサイクルとの接続を持ちます。主要機能の実装場所ではありません。
...
`libs/ui`は「画面だけ」の層ではありません。`KisDocument`、入出力管理、ツール共通処理など、アプリケーションの調整役も含みます。一方、`KisImage`はウィンドウやファイル名を所有せず、画像内容と非同期処理を中心に扱います。
...
画面状態を必要としない画像処理は`libs/image`側、ファイル名やダイアログ、ウィンドウとの調整を必要とする処理は`libs/ui`側から検討します。
...
デスクトップでは`lib/kritaplugins`などから動的に読み込みます。iOSでは動的コード読み込みを使わず、`kis_add_library`が`MODULE`を静的ライブラリーへ変換し、`krita_ios_target_static_plugins`が実行形式へ登録・リンクします。組み込む対象の正本は[initial-plugin-profile.json](../../packaging/ios/manifests/initial-plugin-profile.json)です。

機能を追加するときは、C++クラスだけでなく次の識別子を一組として確認します。
...
描画、文書、ファイル形式、画面動作がOSに依存しない場合は`libs`または`plugins`を所有者にします。OSのライフサイクル、ネイティブファイル選択、入力API、署名・配備との接続だけをプラットフォーム境界へ置くと、デスクトップとモバイルで同じ処理経路を検証できます。

### 4. 安定識別子を確認する

KRA MIME／UTI、設定ディレクトリー、CMakeターゲット、プラグインID、アクションID、デスクトップIDには互換性上の意味があります。表示名やブランド変更と同じ扱いで変更せず、参照元と移行方法を確認します。
...
文書用の全ツールは`nix develop .#docs`にあります。図の生成元は`docs/architecture/*.d2`で、SVGはレビューと通常のMarkdown表示のために追跡します。SVGを直接編集すると生成元とずれるため、D2の生成元を編集して再生成します。
...
新しい主要境界を追加した場合は、全体構造、変更内容から見る場所、該当する実行経路の三か所が整合するように更新します。プラットフォーム固有の詳細手順は、この文書へ重複させず`docs/<platform>/`または`packaging/<platform>/`の文書を正本にします。
