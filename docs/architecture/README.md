...
公開面から除き、現在の公開面1,549ヘッダー、29,989 APIのうち5,548 APIを対応済みとし、未対応
24,441 APIを責務と
所有CMake対象ごとにゼロへ縮小します。
各単位は製品実装前に対象試験の変更なし計画、直接依存、空構築閉包を監査し、構築範囲が責務を
越える場合は所有単位を先に分けます。ヘッダーをコンパイルするだけの試験は挙動契約数に含めません。
複数単位を並列に進める場合は、同じ基準コミットから重ならない公開ヘッダー、実装、試験、
CMake定義を担当票へ固定し、専用Git作業ツリーと専用Ninja木を使用します。統合担当が契約台帳と
進捗文書を一意に所有し、各担当の結果を一件ずつ検証して対応済み基準へ反映します。具体的な担当票、
構築実行許可、引渡し、統合手順は`docs/architecture/DEVELOPMENT.md`に記録しています。
