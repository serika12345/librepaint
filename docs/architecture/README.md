...
取り消し接続である。現行実装が文書全体から取得している取り消し履歴を直接借用し、
履歴操作と変更通知だけを`libs/document/undo`の`kritadocument`が所有する計画とする。
`KisDocument`は履歴と接続の寿命を構成し、文書UIの型を文書側へ公開しない。実装単位は
履歴の現在位置、追加、取消し、マクロ、やり直し破棄、同期通知と借用寿命の契約を持つ。
macOSとLinuxで契約試験を実行し、macOS、Linux、iOS、Android、Windowsで新ターゲットと
製品利用元の構築および依存グラフを確認する。旧ファイルと転送ヘッダーは同じ変更で除去する。
既存の取り消し履歴は操作履歴とQt Widgetsのアクション生成を同じライブラリーで提供する。
最初の単位はLibrePaint内の`kritaui`依存だけを除去し、Qt Widgets依存の分離は文書ドメインを
Qt Widgetsなしで利用できるというR1-G6e全体の完了条件に照らして後続単位で判定する。

現行の`document-state`分類は25クラスであり、文書寿命だけでなくノード操作、選択操作、
表示モデルを含む。後続単位では各クラスの実依存から文書状態、文書表示、別機能の操作接続を
