...
5構成のCMakeターゲットへ接続する。公開ヘッダーは所有ターゲットとエクスポート記号に
加え、別ターゲットが実際にincludeするソースを利用根拠として持つ。主要クラスは宣言、
実装、責務、利用根拠を持つ。プラグインはCMake所有者、JSONメタデータ、登録マクロ、
サービス種別、実行時の利用者を持つ。

`scope`が`representative`の台帳はR1-G2aの形式契約を固定し、`KisDocument`、
`KisImage`、`KisImportExportManager`、PNGインポートを代表項目として記録する。
この範囲は、全件採取前に所有、利用、責務、プラグイン登録の検査規則を確定するための
小単位である。R1-G2の全件採取が完了した時点で`scope`を`complete`へ更新する。
