...
| --- | --- | ---: | --- |
| アプリケーション調整 | 描画 | 58 | `kritaui`内の起動・共有サービスから画像、ブラシ、色処理 |
...
一意に帰属する。この規則で58件の直接includeが現在の確認済み基準になる。内訳は、公開
ヘッダーが画像型を宣言する2件、起動時の具体所有者接続4件、文書・表示操作52件である。
...
操作、メタデータのプラグインを`KoPluginLoader`のサービス種別から読み込む。起動側に残る4件は
3所有者の登録入口とアニメーション書出しに必要な画像接続である。
...
1種類58件の逆方向依存、2ヘッダー2件の内部参照を正本へ照合し、最終状態のゼロ上限を確認する。
