...
`kritadocument`の公開リンク閉包をQt Coreだけへ縮小する。続いて表示を文書UIへ、既存の
具体的なファイル処理を文書ファイル保存へ集約する。形式選択と形式変換は既存の入出力所有に
維持する。パッケージ境界の成立後に残る依存と試験困難性を再評価し、現在必要な根拠を持つ場合だけ
ロジック再構築または抽象化を独立した検査段階として追加する。
