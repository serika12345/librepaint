...
| キャンバス表示 | 入力解釈 | 13 | キャンバスと入力UIのアクション群、管理器、診断接続 |
| 入力解釈 | 描画 | 24 | 入力処理から描画情報とストローク状態 |
| 入力解釈 | リソース管理 | 4 | 入力処理からプリセットとリソース状態 |
| ツール呼出し | 入力解釈 | 7 | ツールと入力UIのアクション群、管理器、プロファイル接続 |
...
一意に帰属する。この規則で123件の直接includeが現在の確認済み基準になる。R1-G6aは、
...
入力標本化と遅延描画キューを`kritatools`へ移した際は3件の描画includeが移設先に残り、
入力とストローク生成が混在する接続元を明示分類したことで11件が入力解釈の根拠として
可視化された。この分類更新は製品includeを追加せず、入力から描画への上限を25件へ更新する。
色採取ストロークの実行所有を描画へ移した後は、UI入力側から画像ストローク接続面への直接参照を
1件除去し、入力から描画への上限を24件へ縮小した。ショートカット照合を`kritainput`へ移した際に
入力UIを独立した所有単位へ移した結果、キャンバス表示から入力解釈への13件とツール呼出しから
入力解釈への7件がパッケージ間参照として可視化された。R1-G6gがこの上限を所有し、UI入力接続と
ツールのキャンバス命令境界へ吸収する完了条件を持つ。

共有ターゲットの保守的射影から生じた責務対は、構造違反基準が実際のincludeを帰属させる。
文書設定画面から入力設定UIへの4経路は共有所有ターゲットにより文書寿命と入力解釈の帰属が
重なるため、1件の未確定射影として根拠を保持する。確認済み違反の上限は根拠のある現在の5責務対を
含む。
...
- `libs/ui`の`kritaui`はアプリケーション、ウィンドウ、文書、キャンバス、ツール共通部をまとめます。
