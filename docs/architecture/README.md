...
MIME集約とプラグイン探索、エラー表現、文書メタデータを内部オブジェクト所有単位として集約する。
実フィルター生成はフィルター本体と同じ公開共有ライブラリーに残す。公開ライブラリー名とシンボル
所有は維持し、責務別のCTestは対応する内部実装だけを直接リンクする。文書UIの挙動契約を追加するときは、
...
除外して、`docs/architecture/public-api-test-contracts.json`の対応と照合します。
...
公開面から除いた29,013 APIのうち418 APIを対応済みとし、現在の未対応28,595 APIを責務と
