...
| キャンバス表示 | 入力解釈 | 1 | `KisCanvas2`が公開する入力アクション群マスク |
| 入力解釈 | 描画 | 24 | 入力処理から描画情報とストローク状態 |
| 入力解釈 | リソース管理 | 4 | 入力処理からプリセットとリソース状態 |
| ツール呼出し | 入力解釈 | 1 | 輪郭ツールが保持する入力アクション群マスクのスコープ |
...
一意に帰属する。この規則で105件の直接includeが現在の確認済み基準になる。R1-G6aは、
...
可視化されたキャンバス表示とツール呼出しから入力解釈への各1件はR1-G6gが所有し、UI入力接続と
...
9責務と5構成の現行ターゲット、5種類105件の逆方向依存、4ヘッダー7件のUI内部参照を
