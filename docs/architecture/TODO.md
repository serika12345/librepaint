...
| R1 | コードパッケージングの改善 | 責務地図、許可依存方針、凝集したCMakeターゲット、変更波及基準 | G0 |
...
8. **R1-G8 変更波及の局所化**: 公開ヘッダー、私有実装、CMake対象、契約試験の変更が再コンパイル、再リンク、修正を要求する範囲を測定し、異なる責務の同居による不要な波及を一つの実装単位で縮小する。
...
目的は、所有者への直接依存を確立したR1-G7とR2-G19bmの結果を入力として、ある責務の変更が
無関係な製品実装や試験の再コンパイル、再リンク、同時修正を要求する範囲を特定し、現在存在する
責務境界に沿って縮小することである。公開契約の変更に必要な利用元更新と、集約対象、公開ヘッダー、
過度な利用要件による偶発的な波及を区別する。

#### R1-G8a 波及基準の採取と最初の実装単位の選定

対象はCMake File APIに現れる製品対象、外部利用される公開ヘッダー、対応する既存CTestとする。
生成した現在状態を正とし、対象名や件数の手書き台帳は維持しない。次の値を同じソースリビジョンと
構築構成から採取する。

- CMake直接辺を逆向きに集計した対象ごとの製品利用元数と試験利用元数。
- Ninja依存データから得る公開ヘッダーごとの再コンパイル翻訳単位数と、所有パッケージ外の利用元。
- 私有実装を変更したときに再リンクされる製品対象と試験対象、および変更なし状態の増分構築計画。
- 最も近い既存契約の直接CMake依存と空構築コマンド閉包。
- 同じ増分操作の経過時間とコンパイラーキャッシュ命中状況。

`scripts/architecture/measure_change_impact.py`は、`ninja -t deps`、`compile_commands.json`、
CMake File APIを読み、任意の製品ヘッダーについて直接取込み元、推移的な翻訳単位、生成ソース、
AUTOMOC、影響対象、リンク対象、所有対象の逆向き利用元、既存契約の閉包を同じ形式で報告する。
全体順位や歴史的上限は継続契約にせず、レビュー可能な一変更の前後値だけを進捗記録へ残す。

macOSの`b85033aced`基準では、対象の逆向き利用元と公開ヘッダーの推移的な波及を別々に調べた。
直接利用元が多い対象の初期値は次のとおりである。

| 対象 | 全利用元 | 製品利用元 | 試験利用元 |
| --- | ---: | ---: | ---: |
| `kritaapplicationui` | 248 | 161 | 87 |
| `kritaimage` | 207 | 34 | 173 |
| `kritaresourcestorage` | 122 | 14 | 108 |
| `kritaapplication` | 77 | 51 | 26 |
| `kritapigment` | 60 | 25 | 35 |
| `kritaglobal` | 59 | 36 | 23 |
| `kritaflake` | 55 | 15 | 40 |
| `kritawidgets` | 52 | 30 | 22 |

`kis_assert.h`、`KoID.h`、`kis_types.h`、`KoColor.h`は、全体検査、識別子、画像所有型、色値という
実利用のある共有契約であるため、利用元数だけを根拠に分割しない。一方、`KoID.h`は識別子と表示名の
公開値契約に加えて、遅延翻訳の格納実装である`TranslatedString`、`StorageType`、`KoIDPrivate`を
完全定義し、Boost、KDE翻訳、`KisLazyStorage`、同期実装を全利用元へ到達させている。この実装詳細は
公開APIを変えずに`KoID.cpp`へ閉じ込められ、既存の`KoIDContractTest`が利用結果を保護するため、
最初の実証単位とする。

変更前の`KoID.h`は直接取込み245ファイル、推移的なコンパイル2147工程である。製品1699工程、
試験448工程、生成252工程のうちAUTOMOC 193工程、影響704対象、直接リンク570件、再リンク閉包703件である。
初期調査値1724件にはベンチマーク25工程が製品として含まれていたため、測定入口はCMake対象の
ソース所有で分類して除外した。
所有対象`kritaglobal`の逆向き利用元は製品36件、試験23件であり、`KoIDContractTest`の直接依存は
`kritaglobalidobjects`と`Qt6::Test`、空構築閉包は5コマンドである。

最初の実装単位は、`libs/global/KoID.h`の私有格納型と、その完全定義を要求する比較・デバッグ処理を
`libs/global/KoID.cpp`へ移す。所有対象は`kritaglobalidobjects`と最終共有ライブラリー`kritaglobal`を
維持する。`KoID`の公開クラス名、構築、複製、代入、`id()`、`name()`、比較、メタ型、デバッグ出力、
クラス配置と記号所有を維持する。対象は共通C++実装であり、macOS、iOS、Linux、Windows、Android Qt 5を
検証範囲とする。

完了条件は次のとおりとする。

- [x] macOSの製品・試験利用元、公開ヘッダー再コンパイル範囲、リンク範囲、契約閉包を再生成できる入口と結果を記録する。
- [x] iOSの現構築木と、同一コミットで成功済みのLinux、Windows、Android Qt 5構築を照合し、条件付き利用元を後続検証範囲へ含める。
- [x] 上位候補を必要な共有契約と偶発的な実装依存へ分類し、最初の一単位について起点ファイル、行先、所有CMake対象、既存契約、公開API、プラットフォーム範囲を固定する。
- [x] 分割根拠と観測契約が成立する`KoID`を選定し、実装へ進む判定を記録する。

#### R1-G8b 最初の局所化実装

R1-G8aで選定した`KoID`一単位だけを変更する。公開ヘッダーから遅延翻訳の格納実装を除き、
既存の値契約とバイナリー配置を維持する。新しい抽象、サービス、工場、対象は追加しない。

macOSの最初の実装では、`KoID.h`から`TranslatedString`、`StorageType`、`KoIDPrivate`の完全定義を
`KoID.cpp`へ移した。`KoID.h`の共有契約範囲は維持し、分離対象の`KisLazyStorage.h`が到達する
推移的コンパイルを2151工程から7工程、生成工程を252件から0件、直接リンク対象を571件から4件へ
縮小した。公開クラス、値契約、記号名、CMake所有を維持し、macOSの完全native検査は879件すべて成功した。
iOSは製品再構築3092工程、最終アプリリンク、未分類0件の静的依存資源監査に成功し、直後の
増分計画は0工程となった。Linux、Windows、Android Qt 5の条件付き利用元と最終リンクも、
x86_64 Linux構築ホストでR1-G8eの統合範囲として確認した。

完了条件は次のとおりとする。

- [x] `libs/global/KoID.h`から`libs/global/KoID.cpp`への私有実装移動と、変更前後の直接利用元、再コンパイル、リンク、空構築閉包を記録する。
- [x] 無関係な製品または試験への波及が一つ以上除去され、必要な共有契約への依存は維持される。
- [x] 識別子、遅延翻訳、複製、代入、比較、デバッグ出力、公開API、ABIが維持される。
- [x] 対象CTest、影響範囲のCTest、`verify-quick`、完全native検査、変更したCMake境界の対応プラットフォーム構築が成功する。
- [x] 一単位の結果を確認して停止し、同じ測定で次の候補へ展開するかを別の検査段階として判断する。

#### R1-G8c 曲線計算実装の局所化

曲線の公開値契約に同居していた私有スプライン算法を画像実装へ閉じ込める。起点は
`libs/image/kis_cubic_curve.h`から`libs/image/kis_cubic_curve_spline.h`への取込みであり、行先は
`libs/image/kis_cubic_curve.cpp`から同じ私有ヘッダーへの取込みである。スプライン実体は既存の
`KisCubicCurve::Data`が所有し、ファイル配置、`kritaimagecubiccurveobjects`と`kritaimage`の所有、
公開クラスと記号を維持する。

変更前の私有スプラインヘッダーは、直接取込み3ファイルから推移的コンパイル1088工程、生成114工程、
影響392対象、直接リンク331対象、再リンク閉包507対象へ到達していた。公開ヘッダーから取込みを除き、
Eigenを`kritaimagecubiccurveobjects`の私有利用要件にした結果、直接取込み2ファイル、推移的コンパイル
2工程、生成0工程、影響2対象、直接リンク1対象、再リンク閉包489対象となった。曲線実装を変更したときの
`kritaimage`下流の必要な再リンクは維持しながら、スプラインテンプレートまたはEigen利用の変更による
無関係な再コンパイルを除去した。

既存`KisCubicCurveContractTest`は、曲線点、整列、編集とキャッシュ無効化、値意味論、評価、転送表、
直列化を観測する。同試験の直接依存からEigenを除き、公開契約だけで利用できることも確認する。

完了条件は次のとおりとする。

- [x] `libs/image/kis_cubic_curve.h`から`libs/image/kis_cubic_curve.cpp`への私有依存移動と、変更前後の再コンパイル、リンク、空構築閉包を記録する。
- [x] `Eigen3::Eigen`を所有対象の私有利用要件にし、契約試験から実装だけの直接依存を除く。
- [x] 公開API、ABI、値意味論、評価結果、転送表、直列化経路を維持する。
- [x] 対象CTest、`verify-quick`、完全native検査が成功し、対象増分計画が0工程になる。

#### R1-G8d 資源種別名の翻訳依存局所化

資源種別キーと表示名の宣言を共有する`libs/resources/KisResourceTypes.h`から、表示名を生成するKDE翻訳の
完全型依存を`libs/resources/KisResourceTypes.cpp`と完全型を実際に使用する利用元へ移す。公開されている
資源種別キー、`KLocalizedString`値、表示名変換関数、記号所有は維持し、前方宣言で値宣言に必要な境界を
表現する。

変更前値は直接取込み174ファイル、推移的コンパイル1355工程、生成100工程、影響568対象、直接リンク
467対象、再リンク閉包655対象である。所有対象`kritaresources`の直接利用元は43対象（製品25・試験18）、
`KisResourceTypesContractTest`の空構築閉包は6コマンドである。既存の表示名契約と保存済み資源キーの
互換試験を観測契約とする。

実装では、起点`libs/resources/KisResourceTypes.h`の`KLocalizedString`完全定義を、行先
`libs/resources/KisResourceTypes.cpp`と`libs/resources/tests/KisResourceTypesContractTest.cpp`へ移し、
公開ヘッダーは`QString`と`KLocalizedString`の前方宣言だけを所有する。完全型の削除直後に
15箇所の不完全型診断を再現し、
翻訳マクロを使う246ファイルと`KLocalizedString`のメソッドを使う6ファイルを、それぞれKDE翻訳の
所有ヘッダーへ直接接続した。`libs/flake/resources/KoGamutMask.h`は、翻訳ヘッダー経由で得ていた
`QObject`をその所有ヘッダーから直接取り込む。

`libs/resources/CMakeLists.txt`で`kritaresourcestypesobjects`の`KF${KF_MAJOR}::I18n`を公開利用要件から
私有利用要件へ縮小し、`libs/resources/tests/CMakeLists.txt`の契約試験だけが翻訳実装を直接要求する。
これにより、資源種別ヘッダー経由の翻訳完全型の再コンパイル経路は1355工程から0工程となり、
翻訳実装の変更影響は明示的な252利用元に限定される。一方、資源キー契約自体の174直接利用元、
467直接リンク対象、655再リンク閉包、所有対象の43利用元は必要な波及として維持する。契約試験の
空構築閉包は6コマンドで、直接依存は`kritaresourcestypesobjects`、Qt Test、KDE翻訳である。

完了条件は次のとおりとする。

- [x] 完全型を使う製品および試験利用元を特定し、各利用元をKDE翻訳の所有ヘッダーへ直接接続する。
- [x] 公開ヘッダーから実装依存を除いた前後の再コンパイル、リンク、空構築閉包を記録する。
- [x] 資源種別キー、表示名、公開API、ABIを維持する。
- [x] 対象CTest、互換試験、`verify-quick`、完全native検査が成功する。

#### R1-G8e 局所化単位の5構成統合

R1-G8bからR1-G8dまでの共通C++変更を、macOS、iOS、Linux、Windows、Android Qt 5の製品構築、
最終リンク、パッケージ境界検査で統合する。x86_64 Linux構築ホストでLinux、Windows、Android Qt 5を
検証し、iOSはG8cとG8dを含む最終状態で再検証する。プラットフォーム固有の実行時契約はR2-G19d-aで
扱う。

統合では、macOSの`./scripts/verify`がパッケージ境界1752対象とCTest 879/879件を含めて
成功した。iOSは最終変更の再構成11工程、`LibrePaint.app`のリンク、未分類0件の
静的依存資源監査に成功し、直後の増分計画は0工程となった。x86_64 Linuxは1766対象の
境界を検査し、233個の製品対象と`krita`実行ファイルを構築し、対象増分計画は作業なしとなった。
Windowsは1670対象の境界を検査し、4155工程で`krita.exe`と全製品部品をリンクした。
Android Qt 5は1642対象の境界を検査し、3903工程で`libkrita_arm64-v8a.so`と
全製品部品をリンクした。増分計画の残りは既存の翻訳生成2工程である。

Linuxの空構築では、公開輸出ヘッダーが対象の公開構築ディレクトリではなく、物理的な旧UIソースの
構築ディレクトリだけに生成される問題を確認した。起点`build/<preset>/libs/ui/kritaui_export.h`から、
行先`build/<preset>/libs/application/kritaui_export.h`を`kritaapplicationui`の正式な生成位置とし、
`libs/ui`の分割オブジェクト対象向けに同一ヘッダーを前者にも同期する境界へ変更した。
公開定義は一つのまま、所有対象の公開利用元と、旧ソース配置をコンパイルする内部対象の両方が
空構築で同じ輸出契約を解決できる。

完了条件は次のとおりとする。

- [x] macOSの完全native検査とiOSの製品構築、最終リンク、資源監査、空増分計画が成功する。
- [x] Linux、Windows、Android Qt 5の製品構築、最終リンク、パッケージ境界検査が成功する。
- [x] G8bからG8dの変更前後値、維持した契約、残る必要な波及を最終記録へ統合する。
- [x] R1-G8とR1の完了条件を満たし、次の作業をR2-G19d-aへ更新する。

R1-G8aの測定基準後の継続範囲は、G8b、G8c、G8d、G8eの4作業単位である。G8bとG8cの構造実装を
完了した時点をおよそ50%の中間確認点とし、G8dとG8eの結果を統合してR1-G8を完了した。

### 完了条件

R1-G1からR1-G7までは、10責務と27の中核所有ターゲットを責務地図へ接続し、5構成の中核ターゲットと全製品ターゲットで
...
- [x] R1-G8の一単位で偶発的な変更波及を測定して縮小し、必要な共有契約と区別できる。
...
### R2-G19a 公開APIの初期調査基盤（完了）

目的は、公開APIの現状を調査し、回帰テスト整備の開始点を作ることである。

- [x] 公開宣言の調査と既存試験の確認を完了した。
- [x] 初期調査用の対応管理を終了し、通常の試験・CMake定義を保守の起点とした。

### R2-G19b 公開APIの初期回帰テスト整備（完了）

目的は、安全なリファクタリングを始めるための回帰テスト群を整備することである。

- [x] 公開APIを対象とした初期回帰テスト整備を完了した。
- [x] 既存試験を責務ごとに保守し、利用者から観測できる結果を基準に整理する開始点を確立した。

対象OSでの未実施検証はR2-G19dで扱う。
...
1. 対象の実装、呼び出し側、仕様、既存試験から、戻り値、状態変化、通知、副作用、失敗条件、
   状態遷移、必要な呼び出し順序、ドメイン上の不変条件を特定する。
2. 試験ごとに振る舞い契約、明示的な互換性契約、宣言形状だけの検査を区別する。
3. 必要な意味論の既存検証を確認し、不足する場合だけ公開操作と観測結果による試験を追加する。
4. 実装詳細の固定、重複検証、不要な試験対象と専用依存を削除する。
5. 対象試験と影響範囲のCTest、高速検査を実行し、変更理由と利用者への影響をレビュー説明に記す。

試験の正本は試験コードとCMake定義とする。宣言ごとの網羅的な対応表や件数目標は、
独立した意味論を持たない宣言まで固定する原因となるため設けない。
意味論が未確定の場合は、その利用場面を設計課題として切り出す。
構築範囲や変更範囲が対象責務を越える場合は、その単位を止めて作業範囲を見直す。

完了条件と検証範囲は次のとおりとする。

- [x] 型特性だけの宣言形状試験251件と、型特性だけになった91ファイルの削除を維持する。
- [x] `ContractTest`への型特性、完全署名、コンパイル時形状検査の再追加を高速検査が拒否する。
- [x] Schema/Enum専用試験の未監査ファイルを0件にする。
...
- [x] TabletTestDialogの宣言形状検査を、接近・離脱のログ表示、消去後の再入力、終了の試験へ置き換える。
      macOSの対象試験・20回反復・高速検査は成功。関連試験の起動不可2件はPROGRESSに記録する。

- [x] 共通値・色処理・ブラシ設定・入力の試験から、重複する型検査、内部補助クラス、
      作業用領域の配置、不要な呼び出し回数と順序の固定を除去した。
      設定の読み書き、変換結果、所有と寿命、通知、旧形式との互換性を動作試験で保護する。
- [x] 色値・色空間登録・パレット・グラデーションの宣言形状専用試験を除去し、
      既存の動作試験へ集約した。色の不透明度・コピー・メタデータ、グラデーション補間を追加した。
- [x] 画像APIの集約signature試験を、ヒストグラムとメモリー統計の初期値試験へ縮小した。

- [x] 画像・図形・SVG文字・画面状態の宣言形状と非公開データ構造の固定を除去し、
      試験内の偽物だけを検証する対象を削除した。メッシュ分割、取り消し・再実行、
      配列の独立性と寿命、ロックの解放を公開操作の結果で検証する。
      macOSの対象26試験が成功。関連試験の依存不足4件はPROGRESSに記録する。

- [x] 共有ライブラリーの公開記号と完全型の直接依存を復旧し、画像・図形・画面部品の所有対象を
      全体構築できる状態へ戻した。標準ブラシ設定の生成処理は独立したsignature検査を削除し、
      寸法・不透明度・流量の読み書きと設定変更通知による再読込を既存のPaintOp試験で検証する。
      画面接続の値構造検査は、チェック欄・数値欄・文字列欄の双方向同期と、範囲・有効・表示状態の
      反映へ置き換えた。macOSで所有ライブラリー4件、対象2試験、以前起動できなかった関連試験4件が成功した。

- [x] 型特性または`static_assert`を含み、実行時検証を一つも持たない宣言形状専用試験251件と
      専用CMake対象を削除した。CMake再構成後のCTest登録は1,020件で、削除済みソースへの参照はなく、
      高速検査が成功した。実行時検証との混在試験は関数単位の整理へ進める。
- [x] 実行時検証と混在していた試験から型特性・完全signature検査、空試験537件、未使用の型別名を除去した。
      型特性だけになった91ファイルと専用CMake対象を削除し、`ContractTest.cpp`の`type_traits`利用を0件にした。
      残存対象を含むCTest 96件はmacOSで成功した。
- [x] 列挙値、内部Traits、モデル役割番号だけを固定するSchema試験5件を削除した。角度選択APIでは
      列挙値の順序に依存していたスクリプト文字列変換を明示的な対応へ直し、有効値の往復と無効値で
      状態を維持する振る舞い試験へ置き換えた。完全な依存構築で露出した直接include不足と、入出力UIの
      補助実装が最終共有ライブラリーへ入らないCMake所有不備を修正した。macOSの対象試験は3回連続成功した。
- [x] ガイド線種の数値固定を実際のペン種別・色・XML往復の検証へ統合し、Qt 6.4以降で保存色を
      復元できない不具合を修正した。マウスボタンによる前景・背景色の選択試験は振る舞い試験として保持し、
      既定ショートカット種別のビット集合だけを固定する試験は削除した。macOSの対象2試験は3回連続成功した。
- [x] 格子種別・線種の整数値と内部三角関数キャッシュのメンバー配置だけを固定する試験を削除し、
      実線・破線・点線・非表示の描画ペン、色、格子種別がXML往復後も維持される検証へ統合した。
      macOSの格子設定試験は3回連続成功した。
- [x] 契約試験の採用単位を利用場面と状態遷移に定め、型特性、コンパイル時形状検査、完全署名別名を
      `ContractTest`へ追加できない高速検査を導入した。明示的な互換性試験には利用者と維持対象の記載を
      必須とし、宣言単位の網羅へ戻らない検査条件を確立した。

関連する既知不具合は独立した変更として扱う。

- [ ] `libs/widgets/KoDialog.cpp`の`showEvent()`が登録する遅延位置調整をダイアログの寿命へ結び付ける。
      表示直後に破棄してからイベント処理を進めると、処理先のオブジェクトが既に破棄されている。
      修正時は表示・破棄・後続イベント処理が安全に完了する公開操作の試験を追加する。
...
- [ ] 実MSVC互換のWindows実行環境で、`winquirks/unistd.h`の利用側から必要な
...
#### R2-G19d-d iPadOS文書提供者の保存契約

iPadOSの文書ピッカーが許可した単一ファイルに、フォルダー全体の書込み権限を要求せず
KRA・ORA・PNG・JPEGを保存できる契約を確立する。[Issue #40](https://github.com/serika12345/librepaint/issues/40)が
実装と検証を追跡する。状態は`complete`である。文書提供者ごとの実機受入れ検査はPR提出後に行う。

範囲は文書ファイルの書込み、バックアップ、自動保存、iOS書出し接続、対応する契約試験と
構築に固定する。他OSの保存方法、保存形式、画像内容、文書状態は維持する。

- [x] 書出し済みのローカル完成物から、選択済みの既存ファイルを削除せずに内容を置き換え、成功時のバイト列と失敗診断を契約試験で固定する。
- [x] iOSは同一ディレクトリーへの一時ファイル作成を要求せず、アプリ領域で完成した出力を選択ファイルへ反映する。
- [x] iOSの隣接バックアップと自動保存をアプリ内の回復領域へ配置し、新規保存先の空ファイルを保存済み文書として扱わない。
- [x] `kis_document_files_test`、影響範囲のnative試験、`verify-quick`、iOS arm64製品構築と静的資源監査を成功させる。実機の保存・再読込みはPR後の受入れ検査として追跡する。

#### R2-G19d-e タッチ操作による入力割当削除契約

キャンバス入力設定で、右クリックを持たないタッチ端末から既存の入力割当を削除できる契約を
確立する。[Issue #39](https://github.com/serika12345/librepaint/issues/39)が実装と検証を追跡する。
状態は`complete`である。

範囲は共有のキャンバス入力設定表示、既存の削除可否判定と削除操作への接続、対応する契約試験と
構築に固定する。入力プロファイル形式、操作識別子、入力解釈、モバイル画面全体の配置は維持する。

- [x] 展開した入力操作に、選択中の割当を主操作で削除できる表示ボタンを設ける。
- [x] 既存の右クリックメニューを維持し、表示ボタンと同じ削除可否判定および削除処理へ接続する。
- [x] 未選択、追加用行、削除不可の必須割当では、表示ボタンとメニュー操作を無効にする。
- [x] 選択した割当が入力プロファイルと表示モデルから削除される利用者向け契約を追加する。
- [x] macOSの対象試験、反復試験、全native検査、`verify-quick`、iOS arm64製品構築と静的資源監査を成功させる。Android検証は、macOSとiPadOSを通常変更の優先対象とする方針に従い、Androidリリース検査または問題発覚時に行う。

#### R2-G19d-f Apple Pencilダブルタップ登録契約

iPadOSがApple Pencilのダブルタップへ割り当てた操作を、表示中のLibrePaintウィンドウで
受け取り、対応する既存操作へ接続する契約を確立する。状態は`complete`である。

範囲は`UIPencilInteraction`の登録寿命、既存の消しゴム・直前プリセット操作への接続、
iOS arm64製品構築と実機受入れに固定する。Pencilの描画イベント、入力プロファイル、
各操作の実装は維持する。

- [x] UIKitが`UIWindowScene`を接続した後に、Qtのメインネイティブビューが属するウィンドウへ`UIPencilInteraction`を登録する。
- [x] ウィンドウの可視化、キー化、前景復帰で、同じメインウィンドウへの登録を再確認する。
- [x] OS設定の「消しゴムに切り替え」と「前回使用したツールに切り替え」を、それぞれ既存の`eraser_preset_action`と`previous_preset`へ接続する。
- [x] iOS arm64製品の最終リンク、静的資源監査、`verify-quick`、実機への更新、登録ログ、Apple Pencilダブルタップによる消しゴム切替を確認する。

#### R2-G19d-g iPad全画面パネル配置契約

iPadのキャンバスのみ表示で、ブラシを縦方向へ密に選択でき、色選択を表示中のキャンバスに
対応する安全領域内で開く契約を確立する。状態は`complete`である。

範囲は`plugins/extensions/iostouchui`が所有するブラシ一覧、色選択パネル、相互排他的な
表示状態、回転時の再配置、iOS arm64製品構築と実機受入れに固定する。デスクトップの
ブラシプリセット表示設定と前景色ダイアログ、資源選択、色変換、描画処理は維持する。

- [x] ブラシ一覧をサムネイルと名前を持つ上から下への一覧にし、パネルの高さを縦スクロールへ使用する。
- [x] ブラシパネルを画面幅の約40%かつ最大480論理画素に収め、キャンバスの空き領域を維持する。
- [x] 色選択を全画面UIの子パネルとして所有し、上部バー直下の右端へ安全領域内で配置する。
- [x] ブラシ、レイヤー、色の各パネルを相互排他的にし、画面回転とキャンバス寸法変更で再配置する。
- [x] `verify-quick`、iOS arm64製品の最終リンク、静的資源監査、実機上の縦横表示、スクロール、色の確定・取消し、外側タップを確認する。

#### R2-G19d-h モバイルクイックピンチ全体表示契約

二指で素早く内向きへピンチして指を離すと、通常の連続拡大縮小・回転からキャンバスを最寄りの
90度単位へ整列した全体表示へ移す契約を確立する。状態は`in_progress`である。

範囲は共有の二指拡大縮小・回転操作、距離・時間・回転量によるクイックピンチ判定、既存の
表示変換への接続、決定的な契約試験、iOS arm64製品構築と実機受入れに固定する。
入力プロファイルの識別子と保存形式、通常速度の拡大縮小、回転、Pencil描画、表示倍率計算は維持する。

- [x] 350ミリ秒以内に二指間距離が開始時の65%以下へ縮み、回転が10度以内で、解放前の戻りが8%以内の操作をクイックピンチとして判定する。
- [x] 遅い縮小、小さい縮小、外向き操作、回転主体、縮小後の反転、取消しを全体表示から除外する契約試験を追加する。
- [x] 既定プロファイルの二指ドラッグ所有者が、二指の解放時だけ現在角度を最寄りの90度単位へ整列し、表示領域に合わせた倍率と中心を適用し、三指への変更と取消しでは判定を破棄する。
- [x] 開始時の角度・倍率・中心位置から全体表示状態までを180ミリ秒の減速曲線で補間し、新しい指操作が始まった場合は現在位置で中断する。
- [x] 対象試験の空構築閉包を5工程に保ち、対象試験、20回反復、共有UIのnative構築、iOS arm64最終リンクと静的資源監査を成功させる。
- [ ] 完全native検査、`verify-quick`、iPad実機上の通常ピンチ・回転・クイックピンチ・誤発動検査を成功させる。

#### R2-G19d-i iPadキャンバス専用文書タイトルバー契約

iPadのキャンバス専用表示で文書名のタブバーとその占有領域を隠し、上部の描画操作バーとキャンバスを
主表示にする。状態は`complete`である。

範囲はキャンバス専用状態の切替、主画面が所有する文書タブバー、iOS arm64製品構築と
実機受入れに固定する。通常表示、デスクトップの文書タブ、複数文書状態、iPad描画操作バーを維持する。

- [x] キャンバス専用表示へ入る前の文書表示方式とウィンドウ属性を保持し、iOSでは枠なしの最大化文書表示へ切り替える。
- [x] キャンバス専用表示を解除したとき、保存したウィンドウ属性、タブ付き文書表示方式、操作設定を復元する。
- [x] `kritaapplicationui`、`verify-quick`、iOS arm64製品の最終リンクと静的資源監査を成功させる。
- [x] iPad実機でキャンバス専用表示の上端、上部描画操作、画面回転、通常表示への復帰を確認する。

#### R2-G19d-j iPad上端ツールバーと全体表示領域契約

iPadの描画操作バーをキャンバス専用表示の上端へ固定し、クイックピンチによる全体表示で
操作バーの下に残る可視領域へキャンバスを収める。状態は`complete`である。

範囲は`plugins/extensions/iostouchui/iostouchui.cpp`のパネル配置から、
`libs/input/KisQuickPinchRecognizer.{h,cpp}`の全体表示幾何と
`libs/ui/actions/input/kis_zoom_and_rotate_action.cpp`の表示変換への接続に固定する。
通常ピンチ、回転、操作バーを隠した状態、Androidの表示領域は維持する。

- [x] 上部描画操作バーを安全領域内の上端と左右端へ余白なしで配置する。
- [x] 操作バーの下端からキャンバス下端までを利用可能な全体表示矩形とし、操作バーを隠した場合はキャンバス全体へ戻す。
- [x] クイックピンチの目標倍率と目標中心を同じ利用可能矩形から決める契約試験を追加する。
- [x] 対象契約試験、共有UIのnative構築、`verify-quick`、iOS arm64製品の最終リンクと静的資源監査を成功させる。
- [x] iPad実機で横向き・縦向きの配置、クイックピンチ、通常ピンチ、操作バーの表示切替を確認する。

#### R2-G19d-k Apple Pencilメインウィンドウ所有契約

Apple Pencilのダブルタップ受付をLibrePaintのメインUIKitウィンドウが所有し、ブラシ一覧の
検索入力などが補助ウィンドウを表示した後も、既存の消しゴム・直前プリセット操作へ接続する。
状態は`complete`である。

範囲は`krita/KisIOSPencilInteraction.mm`の登録先所有と実機上の動的検証に固定する。
既存アクション、Pencil描画、ブラシ選択、キャンバス専用表示、他OSの入力は維持する。

- [x] 起動時にQtのメインネイティブビューからUIKitのメインウィンドウを一度だけ確定する。
- [x] 可視化、キー化、前景復帰の通知では同じメインウィンドウへの登録だけを確認し、通知元の補助ウィンドウへ接続先を移さない。
- [x] 実機ログでブラシ選択前後のダブルタップ通知と既存アクションへの到達を確認する。
- [x] iOS arm64製品の最終リンク、静的資源監査、IPA監査、実機更新と通常表示への復帰後のダブルタップを確認する。
...
目的は、実装ファイルと構築対象が、利用する型・記号・所有ライブラリーを別の取込みやリンク対象から偶然得る状態を解消し、依存方向を各利用元から所有者へ明示することである。

範囲は画像ノード、画素ブラシ、複製paint-op、グラデーションツールの実装と、`kritapixelbrush`、`kritadefaultpaintops_static`、`kritadefaulttools_static`の3対象に固定する。既存のテストソース、公開動作、保存形式は維持する。

- [x] 値型、テンプレート、インライン処理、標準ライブラリー、Qt型を使う各ヘッダーと実装が、その所有ヘッダーを直接取り込む。
- [x] 対象実装から未使用または過度に広い取込みと未使用宣言を除き、Qt 5を含む対応環境で必要な完全型を維持する。
...
- [ ] `KisStorageChooserWidget.h`から`KisStorageChooserDelegate.h`への公開include互換は、直接includeへの
      移行期限と公開APIの互換終了版を定めた後に削除する。
