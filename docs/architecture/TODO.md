...
増分計画は0工程となった。Linux、Windows、Android Qt 5の条件付き利用元を確認する製品構築は、
x86_64 Linux構築ホストで行うこの検査段階の残作業である。
...
- [ ] 対象CTest、影響範囲のCTest、`verify-quick`、完全native検査、変更したCMake境界の対応プラットフォーム構築が成功する。
...
- [ ] 完全型を使う製品および試験利用元を特定し、各利用元をKDE翻訳の所有ヘッダーへ直接接続する。
- [ ] 公開ヘッダーから実装依存を除いた前後の再コンパイル、リンク、空構築閉包を記録する。
- [ ] 資源種別キー、表示名、公開API、ABIを維持する。
- [ ] 対象CTest、互換試験、`verify-quick`、完全native検査が成功する。
...
- [ ] macOSの完全native検査とiOSの製品構築、最終リンク、資源監査、空増分計画が成功する。
- [ ] Linux、Windows、Android Qt 5の製品構築、最終リンク、パッケージ境界検査が成功する。
- [ ] G8bからG8dの変更前後値、維持した契約、残る必要な波及を最終記録へ統合する。
- [ ] R1-G8とR1の完了条件を満たし、次の作業をR2-G19d-aへ更新する。

R1-G8aの測定基準後の継続範囲は、G8b、G8c、G8d、G8eの4作業単位である。G8bとG8cの構造実装を
完了した時点をおよそ50%の中間点とし、G8dの実装開始前に停止する。
...
- [ ] R1-G8の一単位で偶発的な変更波及を測定して縮小し、必要な共有契約と区別できる。
