...
void KisBezierUtilsContractTest::evaluatesCurveAndDerivatives()
{
    ...
    QVERIFY(closePoint(KisBezierUtils::bezierCurve(QList<QPointF>{p0, QPointF(2.0, 2.0), p3}, 0.5), QPointF(2.0, 1.5)));
    QVERIFY(closePoint(KisBezierUtils::bezierCurve(QList<QPointF>{p0, p1, p2, p3}, 0.5), QPointF(2.0, 1.5)));
}

void KisBezierUtilsContractTest::subdividesAndClassifiesCurves()
{
    const QPointF p0(0.0, 0.0);
    const QPointF p1(1.0, 2.0);
    const QPointF p2(3.0, 2.0);
...
void KisBezierUtilsContractTest::linearizesAndMergesOrderedSteps()
{
...
void KisBezierUtilsContractTest::measuresLengthAndMapsArcProportions()
{
    const QPointF p0(0.0, 0.0);
...
void KisBezierUtilsContractTest::nearestPointReportsParametersButLineShortcutLeavesOutputsUntouched()
{
    ...
    const QList<QPointF> cubicLine{QPointF(0.0, 0.0),
                                   ...
                                   QPointF(10.0, 0.0)};
    QVERIFY(closeScalar(KisBezierUtils::nearestPoint(cubicLine, QPointF(4.0, 3.0), &distance, &nearest), 0.4, 1e-4));
    QVERIFY(closeScalar(distance, 3.0, 1e-4));
    QVERIFY(closePoint(nearest, QPointF(4.0, 0.0), 1e-4));
}

void KisBezierUtilsContractTest::findsAllLineIntersectionsAndNearestAnchor()
{
...
void KisBezierUtilsContractTest::mapsPatchesAndEditsControlPoints()
{
