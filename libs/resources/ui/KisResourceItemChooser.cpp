...
KisResourceItemChooser::KisResourceItemChooser(KisResourceUiDescriptor descriptor, QWidget *parent)
    : QWidget(parent)
    , d(new Private(std::move(descriptor)))
{
    // Tag manager
    d->tagFilterProxyModel = new KisTagFilterResourceProxyModel(d->resourceType, this);
    d->tagFilterProxyModel->sort(Qt::DisplayRole);
    d->tagManager = new KisResourceTaggingManager(d->resourceType, d->tagFilterProxyModel, this);
