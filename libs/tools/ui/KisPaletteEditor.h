 ...
 * This class coordinates palette resource and model changes. The application
 * supplies the active document storage and the parent for user prompts.
 *
 * Changes made in this class won't be done to the palette if the palette is
 * read only (not editable, isEditable() == false)
 */
class KRITATOOLSUI_EXPORT KisPaletteEditor : public QObject
{
    Q_OBJECT
public:
    struct PaletteInfo;
...
public:
    ...
    void setDocumentContext(QWidget *dialogParent, const QString &linkedResourcesStorageId);
