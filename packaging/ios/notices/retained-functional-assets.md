...
continue to apply according to their terms. `ios-image-licenses.json` records
the explicit groups' paths, license expressions, and sources, as well as the
accepted embedded-metadata license URL.

## Creative Commons functional artwork

The application retains 703 SVG/SVGZ images whose `cc:Work` metadata directly
links to CC-BY-SA-4.0. The audit rediscovers them from the current QRC inputs
and accepts the canonical CC-BY-SA-4.0 license URL recorded in each file.
...
Iaconelli, and others. The local source paths are listed in the manifest. The
upstream license evidence and the Krita import commits are:
...
the preferred source form. The upstream notice is:
...
The provenance of the 210 root copies is documented by four upstream Krita
import commits:
...
LibrePaint's CC0 artwork supplies the application-facing brand surfaces. The
macOS DMG background uses the same LibrePaint splash. Functional upstream
artwork retains its original license wording and factual attribution.
