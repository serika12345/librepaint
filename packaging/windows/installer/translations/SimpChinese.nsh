...
LangString RemovingShellEx ${CURRENT_LANG} "正在删除 LibrePaint 文件资源管理器插件..."
LangString RemoveShellExFailed ${CURRENT_LANG} "无法删除 LibrePaint 文件资源管理器插件。"
LangString RemoveShellExDone ${CURRENT_LANG} "成功删除 LibrePaint 文件资源管理器插件。"
...
LangString SectionShellExDesc ${CURRENT_LANG} "安装此插件后，Windows 文件资源管理器即可显示 LibrePaint 文件的缩略图和属性信息。$\r$\n$\r$\n版本: ${KRITASHELLEX_VERSION}"
...
LangString ShellExLicensePageHeader ${CURRENT_LANG} "许可证协议 (LibrePaint 文件资源管理器插件)"
...
LangString DesktopIconPageDesc2 ${CURRENT_LANG} "选择是否在桌面上创建启动 LibrePaint 的图标快捷方式:"
...
LangString Msg64bitOn32bit ${CURRENT_LANG} "本机正在运行 32 位版本的 Windows，但本安装程序提供的是 64 位版本的 LibrePaint，它只能在 64 位版本的 Windows 下运行。请获取 32 位版本的 LibrePaint。"
LangString Msg32bitOn64bit ${CURRENT_LANG} "你正在 64 位版本的 Windows 下安装 32 位版本的 LibrePaint，这将降低程序性能。我们强烈建议你改为安装 64 位版本的 LibrePaint。$\n$\n仍要继续安装 32 位版本的 LibrePaint 吗？"
...
#
LangString MsgKritaSameVerReinstall ${CURRENT_LANG} "检测到本机已经安装了 ${KRITA_PRODUCTNAME} ${KRITA_VERSION_DISPLAY}。$\n本安装程序将重新安装此版本的 LibrePaint。"
LangString MsgKrita3264bitSwap ${CURRENT_LANG} "检测到本机已经安装了$KritaNsisBitness位版本的 Krita ($KritaNsisVersion)。本安装程序将用${KRITA_INSTALLER_BITNESS}位版本的 LibrePaint ${KRITA_VERSION_DISPLAY} 将其覆盖。"
LangString MsgKritaNewerAlreadyInstalled ${CURRENT_LANG} "检测到本机已经安装了$KritaNsisBitness位版本的 Krita ($KritaNsisVersion)。如需安装 LibrePaint (${KRITA_VERSION_DISPLAY})，请先手动卸载已有版本，然后再次运行此安装程序。"
LangString MsgKritaRunning ${CURRENT_LANG} "检测到 LibrePaint 正在运行。请在关闭 LibrePaint 后再次运行此安装程序。"
LangString MsgUninstallKritaRunning ${CURRENT_LANG} "检测到 LibrePaint 正在运行。请在关闭 LibrePaint 后再次运行此卸载程序。"
